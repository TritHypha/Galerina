...
LogicN **ext** package — `packages-logicn/logicn-ext-tmf` — under the owner decision of 2026-06-16 to
implement it engine-first **inside LogicN** (a new ext package, *not* the gated TritMesh product repo).

This doc is the authoritative in-KB home for the engine. The format specs it implements are vendored into
`packages-logicn/logicn-ext-tmf/spec/` (provenance + R&D upstream pin in that folder's `PROVENANCE` note);
this KB doc is the discoverable, graph-indexed summary. It supersedes the "`.tmf` engine = R&D-only" line
in earlier copies of `logicn-rd-adoption-2026-06-16.md` (now corrected there).

## Where it sits — core vs ext vs R&D

LogicN's long-standing rule is **govern-don't-absorb**: no crypto enters LogicN **core**. That still holds.
The `.tmf` engine is an **ext** package — the thing LogicN *governs* — so building it in-repo does not breach
the core boundary. The boundary is now three-way:

- **LogicN core** — the language + compiler + governance verifier. No crypto. Unchanged.
- **LogicN ext (`logicn-ext-tmf`)** — the deterministic `.tmf` engine. New, in-repo, this doc.
- **R&D (`LogicN-R-AND-D`)** — the authoring/scratch repo for the specs and research. Upstream of `ext`.

## Crypto-on-core (`LLN-SUBSTRATE-001`) — the hard line

Every primitive runs **bit-exact on a deterministic digital core**: SHAKE256 via `node:crypto`;
ML-DSA-65 / ML-KEM-768 via `@noble/post-quantum` (already a LogicN dependency). **No photonic crypto** —
analog optics (≤~10-bit, error-tolerant) cannot carry an avalanche primitive; SHA-256/SHAKE256 are already
Grover-safe so there is nothing to "speed up" by moving them off-core. See `logicn-substrate-failure-model.md`
and `logicn-quantum-resistance-posture.md`. This was independently re-derived from both the photonic-hashing
and the lattice-crypto literature (see `logicn-rd-adoption-2026-06-16.md` §1).
...
ZERO-TRUST      three-valued verdicts allow / deny / unknown; unknown → deny  (LLN-GOV-3VL-001)
...
## What LogicN governs (not absorbs)

The engine is the **governed object**; the governance lives in LogicN proper:
- **Verify-before-decrypt key release** — the K3 gate `keyRelease(integrityOk, authenticityOk, govVerdict)`,
  landed as `tests/patterns/pattern-10-verify-before-decrypt-gate.lln` (adoption ledger U1).
- **No cleartext semantic embedding across a trust boundary** — `logicn-privacy-embedding-egress.md` (U2/#204).
- Crypto effect typing + PQ/hybrid requirement — `LLN-CRYPTO-PQ-001`, `logicn-quantum-resistance-posture.md`.
