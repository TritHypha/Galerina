...
- **Flow-level `invariant { ensure … }`** (R&D 0040 DbC): parser + governance-verifier (`verifyInvariantBlock`,
  `tryStaticEval`, `LLN-INV-001/002/003`) + interpreter + wat-emitter; fail-closed at the single exit, all tiers.
- **Monotonic emergency `policy{}` overlay** (DRCM Phase 4): `LLN-MONO-001/003`, `validateTransitionMonotonicity` —
...
- **`limits{}`** = the committed-arena memory bound (no `memory.grow` → trap). **Taint / info-flow** = the
  value-state checker + `LLN-SECRET-*` / `LLN-PRIVACY-002`. **`liability{}`-on-Auto** (C-003) = deterministic
...
|---|---|---|---|
| **Static info-flow / structural** | `Cardholder_Data never_touches PublicTelemetryLog` | **Compile-time PROOF → block the build** (fail-closed) over the module taint graph | already the taint system (`LLN-SECRET-*`/`PRIVACY-002`); net-new = the *declarative module-wide* form |
...
Diagnostics: `LLN-INV-MODULE-001..` (or extend `LLN-INV-*`). **Tiering matters for cost:** the static-proof tier
...
1. **"Dynamically LOOSEN invariants in low-risk state" → ❌ FAIL-OPEN.** An invariant dropped in "low-risk" mode
   ...
   loosen** — which is precisely the shipped emergency-overlay rule (`LLN-MONO-001/003`, escalate-never-de-escalate).
...
3. **Host-side `LogicNAutoInvariantEngine` (a TS that reads `wasm.exports.readTotalCredits()` and decides) →
