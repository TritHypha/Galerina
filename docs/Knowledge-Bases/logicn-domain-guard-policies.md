...
> **Disambiguation — Two distinct "policy" concepts exist in LogicN:**
...
> - **Emergency Policy Overlay** (`policy { emergency { on X { deny Y } } }`) — runtime monotonic overlay, inline between `contract {}` and `{ body }`. See `logicn-governance-rules.md` S-008.
...
──────────────────────────────────────────     ───────────────────────────────────────
governance/policies/invoicing_guard.lln        flows/processInvoice.lln
...
4. If any local declaration exceeds the ceiling → hard build fault (LLN-GOV-004 / LLN-LIMIT-001)
...
**With domain guard policies:** the Governance Verifier loads `InvoicingDomainGuard`, checks that `filesystem.wipe_all` is not in `permitted_effects`, and rejects the build with `LLN-GOV-004`.
...
│  governance/policies/invoicing_guard.lln              │
...
│  ❌ NO → LLN-GOV-004: Policy Violation               │
...
```lln
;; File: governance/policies/invoicing_guard.lln
...
**Compiler response:**
```
LLN-GOV-004: Policy Violation. The effect 'filesystem.delete_logs' is
...
**Compiler response:**
```
LLN-LIMIT-001: Resource bounds exceeded. Requested max_memory (16MB)
...
A contract without `[conforms_to: ...]` is **unlocked** — it falls back to standard Governance Verifier rules (effects deny-by-default, etc.). High-trust modules that declare `effects { filesystem.write }` or similar without a domain guard emit `LLN-GOV-019: Unbound contract in high-trust module`.
...
| Code | Description |
|---|---|
| `LLN-GOV-004` | Policy violation — effect/capability not in `permitted_effects`/`permitted_capabilities` |
| `LLN-LIMIT-001` | Resource bounds exceeded — `limits {}` value exceeds policy `enforced_limits` ceiling |
| `LLN-GOV-019` | Unbound contract in high-trust module (no `[conforms_to: ...]`) |
...
- Parse top-level `policy Name { permitted_effects {} permitted_capabilities {} enforced_limits {} }` in `.lln` files
- Load from `governance/policies/` directory at compile time (or from `logicn.policy.lln` in project root)
...
- Emit `LLN-GOV-004` / `LLN-LIMIT-001` on violations
...
├── governance/
│   └── policies/
│       ├── invoicing_guard.lln      ← gateway.charge only, 4MB
│       ├── auth_guard.lln           ← database.read, secret.read, 32MB
│       ├── analytics_guard.lln      ← database.read, network.outbound, 128MB
│       └── admin_guard.lln          ← unrestricted (requires architecture review)
├── flows/
│   ├── invoicing/
│   │   └── charge.lln               ← contract [conforms_to: InvoicingDomainGuard] { ... }
│   ├── auth/
│   │   └── verify.lln               ← contract [conforms_to: AuthDomainGuard] { ... }
│   └── analytics/
│       └── report.lln               ← contract [conforms_to: AnalyticsDomainGuard] { ... }
```
...
| Topic | Document |
|---|---|
| Emergency policy overlay (separate concept) | `logicn-governance-rules.md` S-008 |
| Governance rules C-005 (AI proposal pipeline) | `logicn-governance-rules.md` |
| Contract authoring guide | `logicn-contract-authoring-guide.md` |
| DRCM — V_DPM bitmask gates | `logicn-deterministic-runtime-containment.md` |
