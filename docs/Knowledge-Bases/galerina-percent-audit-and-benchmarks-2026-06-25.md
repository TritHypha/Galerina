...
## Honest read
Galerina **trades raw speed for governed trust** — that is the design, not a regression. Where the emitter produces
...
workloads (so more benchmarks have a production WASM row at all), not optimizing the interpreter. Galerina beats the
Python floor on 2/13 unit-aligned interpreter benchmarks today.

*Source: `galerina-devtools-benchmarks` `compare.mjs` over `results/latest.json` (2026-06-25); supersedes the v2
audit's table.*
