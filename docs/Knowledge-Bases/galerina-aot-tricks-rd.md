...
> that Galerina has literal-const substitution (`staticConsts`) but **NOT** const-*expression* folding or
...
## The ask
(1) What AOT tricks do Rust/C++/Zig/Go use, and which can Galerina adopt? (2) Can the tower-citizen
...
## Part A — classical AOT tricks → honest Galerina mapping (the genuinely adoptable set)
These are real and the right place to look. Confidence flagged; **VERIFY-BEFORE-BUILD** before any of them.

| Trick | Real? | Galerina today | Adopt? |
|---|---|---|---|
...
| **Monomorphization** | ✅ | the WASM/bytecode tiers already emit monomorphic i32 code; the tree-walker is dynamically tagged | Partial — already monomorphic where it matters; full generic-flow specialization only if Galerina grows rich generics. |
...
**Recommendation (when you greenlight a build):** general **constant-folding + propagation + branch-folding + DCE** is the single best classical AOT lever for Galerina — high value, low risk, additive, and it composes with the production-tier lowering already underway (`forEachStmt`→WASM shipped `c6c2896`). It does NOT need new language surface.
...
- **D4 — matrix-chain "fusion" `A₁·A₂·A₃` DENSIFIES** (measured **39× denser**). For sparse reachability
  ...
  that is the for/where data-mask, see [[galerina-for-where-mask-verdict]], not this.)
...
**MeshQL shortest-path / reachability precompute is already PARKED** ([[galerina-tritmesh-meshql-shortest-path-parked]]):
...
## See also
`scripts/rd-aot-tensor-precompute-proof.mjs` · [[galerina-for-where-mask-verdict]] ·
[[galerina-treewalker-photonic-governance-rnd]] · `galerina-tritmesh-meshql-shortest-path-parked.md` ·
