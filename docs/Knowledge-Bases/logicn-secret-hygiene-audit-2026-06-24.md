...
|---|---|---|
| `logicn-ext-tmf` `container.ts:64` / `history.ts:92` / `kemdem.ts:65` `bytesEqual` | digests, chain roots, the **keyed `ctxCommitTag` MAC** | **constant-time** — routed through `crypto.timingSafeEqual` (the `0033(c)` fix), length pre-checked (lengths are protocol-fixed/public) |
...
| `cli.ts:633` `hash1 !== hash2` | two GIR hashes of the **same local source** | non-secret (build-determinism self-check, LLN-BUILD-001) |
...
**Key files:** `logicn-ext-tmf/src/{container,history,kemdem}.ts` (constant-time `bytesEqual`, zeroize) · `logicn-core-compiler/src/wat-emitter.ts` (`:576-619` B2b zero-on-exit, `:569-573` G5b intrusion-wipe, `:3118+` `#70` single-exit) · `attestation.ts` (Ed25519/ML-DSA verify).
