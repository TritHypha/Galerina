# Startup And Boot Warmup

## Purpose

Galerina treats startup performance as a planning problem, not a runtime guessing
problem.

The core rule is:

```text
Do expensive thinking at build/check time.
Do only verified loading at boot time.
```

This supports fast, predictable startup without claiming that Galerina is
automatically faster than mature runtimes.

## Boot Profile

A Galerina production build should be able to generate a verified boot profile:

```text
build/galerina/boot-profile.json
```

The boot profile contains deterministic startup facts:

```json
{
  "schema": "galerina.boot-profile.v1",
  "profile": "production",
  "entry": "boot.spore",
  "targets": ["cpu", "wasm"],
  "routeGraphHash": "sha256:...",
  "policyGraphHash": "sha256:...",
  "schemaValidatorHash": "sha256:...",
  "packageGraphHash": "sha256:...",
  "runtimePlan": {
    "loadMode": "minimal",
    "eagerPackages": [
      "galerina-core",
      "galerina-core-runtime",
      "galerina-core-security",
      "galerina-framework-app-kernel"
    ],
    "lazyPackages": [
      "galerina-ai",
      "galerina-data-search",
      "galerina-target-gpu"
    ]
  },
  "cache": {
    "safe": true,
    "contentAddressed": true,
    "secretsCached": false
  }
}
```

At boot, Galerina verifies the profile and loads the smallest safe production
surface first.

The profile may be generated by future commands such as:

```bash
Galerina check --profile production
Galerina build --profile production
Galerina warmup --profile production
```

## Startup Phases

| Phase | What Happens | Goal |
| --- | --- | --- |
| Cold boot | Load minimal runtime, config handoff, route table and security table | App becomes ready quickly |
| Safe warmup | Preload validators, common routes, CPU feature plan and safe caches | First real requests are faster |
| Deferred warmup | Load AI, search, reports, graph tools, benchmarks and optional targets later | Heavy features do not delay readiness |

## Generated Startup Artefacts

Build/check time may generate:

```text
build/galerina/boot-profile.json
build/galerina/routes.json
build/galerina/security-policy.json
build/galerina/schema-validators.json
build/galerina/effects-map.json
build/galerina/runtime-plan.json
build/galerina/startup-report.json
```

Runtime should verify these artefacts before using them.

## Galerina Boot Snapshot

Galerina may later support a boot snapshot. For the first design, this should be
a verified bundle of deterministic startup artefacts, not a raw runtime memory
dump:

```text
.galerina/cache/
  boot-profile.json
  route-table.bin
  policy-table.bin
  validators.bin
  package-graph.bin
  target-plan.json
  startup-report.json
```

A future native runtime may support a true runtime snapshot. Until then, the
safer model is to verify build artefacts, load minimal runtime state and warm
optional parts after readiness.

## Safe Cache Rule

Cache only deterministic, non-secret, rebuildable startup data.

Safe to cache:

- route graph
- schema validators
- package graph
- compiled safe parsers
- CPU/WASM target plan
- generated reports
- AI-readable project map

Do not cache by default:

- API keys
- session tokens
- payment tokens
- private keys
- raw request bodies
- authorization decisions
- database query results
- external API responses

Any startup cache must be bounded, content-addressed where practical, safe to
delete, safe to bypass and never required for correctness.

## Minimal Production Package Loading

Production boot should use:

```text
package-galerina.json
galerina.lock.json
boot-profile.json
```

Only packages required by the selected profile should load during cold boot.
Development, benchmark, AI, search, report-generation and graph tools should be
lazy or excluded unless explicitly required by the profile.

## Commands

Suggested commands:

```bash
Galerina startup:plan --profile production
Galerina startup:warm --profile production
Galerina startup:report
Galerina serve --profile production --use-boot-profile
```

Example output:

```text
Galerina Startup Plan

Profile: production
Mode: preplanned
Routes: 42 loaded from route-table
Policies: 18 loaded from policy-table
Schemas: 31 validators prebuilt
Packages loaded at boot: 7
Packages lazy-loaded: 12
Secrets cached: no
Cache required for correctness: no
Startup risk: low

Report:
build/reports/startup-report.json
```

## Design Statement

Galerina should be fast to start because it knows the safe startup path before the
application starts.

This means:

- routes are already known
- schemas are already known
- policies are already known
- effects are already known
- package load plan is already known
- target plan is already known
- startup cache contents are safe and rebuildable

The goal is predictable, explainable, measurable and reportable startup.
