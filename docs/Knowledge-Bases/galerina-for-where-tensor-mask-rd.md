...
2. **"`ntt_mul`, 512-bit lanes" — `ntt_mul` is fabricated** (already recorded in 0035; the real op is
   `tmacVector`, an O(count) loop in `galerina-tower-citizen`). SIMD lanes are a HW property — EXCLUDED here.
...
See [[galerina-treewalker-photonic-governance-rnd]].
...
## See also
`scripts/rd-for-where-mask-proof.mjs` · [[galerina-treewalker-photonic-governance-rnd]] ·
`galerina-tree-walker-speed-and-photonic-governance.md` · 0025/0035 done-reports · `c6c2896` (forEach→WASM).
