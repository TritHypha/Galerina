...
| 22 | GC-free O(1) bump/arena alloc (`ptr += size`, reset `ptr = HEAP_BASE`) | **SOUND** | Genuinely shipped (`wat-emitter.ts`: `WAT_HEAP_BASE=1024`, `$__lln_heap`, per-flow reset, secret-zeroing). O(1) per-alloc; technique is standard (region allocators) — Galerina's net-new is **governing** it | `wat-emitter.ts`; Gay & Aiken 1998 |
...
**Confirmed in the shipped codebase.** `packages-galerina/galerina-core-compiler/src/wat-emitter.ts` declares `export const WAT_HEAP_BASE = 1024;` (line 323) and emits the monotone bump pointer `(global $__lln_heap (mut i32) (i32.const ${WAT_HEAP_BASE}))` (line 495); record construction advances it (`ptr += size`), and B2 (R&D 0055) emits a per-flow reset rebasing `$__lln_heap` to `WAT_HEAP_BASE` — the literal `ptr = HEAP_BASE` snap-back — with B2b/G5 zeroing the reclaimed region for secret hygiene. Per-alloc cost is O(1) amortized; no tracing GC → no GC pause.
...
64. `C:\wwwprojects\Galerina\packages-galerina\galerina-core-compiler\src\wat-emitter.ts` — `WAT_HEAP_BASE=1024` (L323), `$__lln_heap` bump pointer (L495), B1 arena→maxPages, B2 per-flow reset, B2b/G5 zeroing. (Confirmed in this R&D.)
