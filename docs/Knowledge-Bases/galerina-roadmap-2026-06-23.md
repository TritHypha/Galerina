...
> **[galerina-roadmap-and-percent-audit-2026-06-23-eod.md](galerina-roadmap-and-percent-audit-2026-06-23-eod.md)** (~**77%**).
...
[galerina-roadmap-and-percent-audit-2026-06-23-eod.md](galerina-roadmap-and-percent-audit-2026-06-23-eod.md) (overall **~77%**,
...
> the unknown→INDETERMINATE-fail-closed thesis) · `0085` RAG-vulnerabilities `GALERINA_SECURITY_RULEBOOK` reconciliation +
> RAG/LLM-retrieval threat class · `0078` OCSP staple-caching (availability vs fail-closed). Absorb verdicts to the
> [R&D results log](galerina-rd-results-log.md) as they land.
...
- **ML-DSA-65 hybrid `.lmanifest` signing** (#34) — **UNBLOCKABLE 2026-06-23**: the signing CODE is already
  ...
  ([#34 ceremony](galerina-34-offline-key-ceremony-runbook.md)). **`galerina keygen --hybrid` SHIPPED 2026-06-23**
  — the ceremony's keygen step is now a CLI command (wires the shipped hybrid keygen; Ed25519 PEM byte-compatible
  + ML-DSA-65, written to `.env.galerina-signing` 0600; round-trip + fail-closed tested via the real CLI). Residual:
  the owner runs `keygen --hybrid` air-gapped; remaining follow-ups = a **`galerina sign` CLI** (wrap
...
[outstanding-rd-and-todos](galerina-outstanding-rd-and-todos-2026-06-23.md).
