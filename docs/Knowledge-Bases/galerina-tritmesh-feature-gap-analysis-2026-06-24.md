...
1. **Deny-by-default tenant-isolation border** (P3, `SPORE-TENANT-001/002`) — **highest-value single item.** Vault tenancy defaults to `tenant_scoped`; `governance-verifier.ts` refuses to mint a manifest for a tenant_scoped vault whose data-access effect is not parameterized by the caller's proven `S_user` — a fail-closed **compile error**. Kills IDOR / OWASP-A01 at compile time. Capability intersection, NOT an AST/query-string rewriter (Galerina does not own the MeshQL string). [M]
