...
|---|---|---:|---|---|---|
| A | **`//spore:` generated comment tier** | **60%** | LOW | `//`→`comment`, `///`→`docComment`, `;;`→`govComment` (manifest-wired, `manifest-generator.ts:432`); 4 distinct comment scans in `lexer.ts` | `//spore:` collapses into plain `//` (the `//` scanner swallows `@`). Add a `genComment` token + a `peek(2)==='@'` branch before the `//` branch (~15 ln, additive, fail-closed) |
| B | **Hardware `//spore:WARN` (uncertainty)** | **70%** | LOW | `HARDWARE_TRUST_PROFILES` (~40 targets, `type-registry.ts:455`) wired into the verifier | Unknown hardware target is silently `continue`d (`governance-verifier.ts:1662`) → make it a yellow `SPORE-HW-004` warning (advisory, non-gating) |
...
| F | **`contract.architecture{volatility,depends_on}` + Stable-Deps** | **8%** | MED | the sub-block dispatch idiom (~30 siblings, `parser.ts`); generic `parseContractSubBlock`; **`SPORE-GOV-013` is the exact structural twin** of the Stable-Deps rule (caller/callee property-conflict walk, `governance-verifier.ts:1524`) | a parse stanza (fail-closed unknown volatility) + a cross-flow `verifyArchitectureStability` pass (`SPORE-ARCH-001`) modeled on GOV-013 |
...
- **1a. `//spore:` generated comment tier** ✅ `1804557` — `genComment` token, ordered before `//`, fail-closed.
- **1b. Hardware `//spore:WARN`** ✅ `5d8d611` — unknown-target → yellow `SPORE-HW-004`.
- **1c. Cyclomatic-complexity metric** ✅ `45bc0a5` — `//spore:COMPLEXITY` (silent at 1), surfaced in `galerina deps`.

**Phase 2 — MEDIUM, read-only / parse-only — IN PROGRESS:**
- **2a. dependency report** ✅ `1a57761` — shipped as `galerina deps <file> [--flow X]`: `//spore:USES` (upstream) /
  `//spore:USEDBY` (downstream "dependants") / `//spore:IMPACT` (transitive blast-radius; 0 = safe-to-delete), from the
...
- **2b. `contract.architecture {}` parse-only** ✅ `<this commit>` — registered contract section + fail-closed
  `SPORE-ARCH-001` volatility value check; the *authored* `depends_on`/`volatility` (vs observed `//spore:USES`).
...
- **3b. Stable-Dependencies enforcement** ✅ `<this commit>` — `SPORE-ARCH-002`, always a hard error (decision #5),
  on the observed call graph; only declared-volatility flows participate.
- **3c. SOURCE WRITER** ✅ — `galerina deps --write` (rewriteGeneratedComments): silently overwrites only `//spore:` lines, idempotent, fail-closed; unit-tested
  (decision #3, done).
- **3e. WHOLE-APP WRITER** ✅ `<this commit>` — `galerina deps --all [dir] [--write]` refreshes `//spore:` across EVERY `.spore`
...
- **3f. BUILD AUTO-REFRESH** ✅ `<this commit>` — `galerina build --package <dir>` refreshes the package's `//spore:` blocks by
  DEFAULT (owner: "it does it anyway"); `--no-refresh` opts out for reproducible CI. Scoped to the package's own `src` tree
  (`dirname(entry)`), never the wider repo; only `//spore:` lines touched. (Single-file `galerina build <file>` stays pure —
...
**Phase 4 — polish:** state-mutability metric (`//spore:Mutates`), central Governance-Registry index (decision #2),
pre-commit hook ✅ template shipped at `scripts/hooks/pre-commit` (the "keep build pure + use a hook" alternative —
runs `galerina deps --all --write` + re-stages changed `.spore`; set `GALERINA_APP_DIR` to your src root).

**STALENESS GATE — opt-in form SHIPPED** ✅ `<this commit>` — `galerina deps --all [dir] --check` re-derives every
flow's block and **exits 1** if any in-file `//spore:` differs from the freshly-computed one (CI fails when someone
...
0045 forward-design #1. **Design constraint for that build:** only flag files that ALREADY carry at least one `//spore:`
line (opted-in). A file with NO `//spore:` is NOT stale — demanding metadata everywhere would fail every fixture/example
...
**CLI ergonomics** ✅ `<this commit>` — added the short bin alias **`spore`** alongside `galerina` (package.json `bin`); both
point at `galerina.mjs`. Takes effect after `npm link` / global reinstall (Windows gets an `spore.cmd` shim).

## R&D 0045 decisions (owner, 2026-06-19)
1. **Token = `//spore:`** ✅ RESOLVED (owner picked it to mirror the `.spore` prefix). Shipped + all artifacts renamed `//@`→`//spore:`.
2. registry vs distributed — still OPEN — Phase 4.
3. **Human edits a generated line → SILENTLY OVERWRITE** ✅ RESOLVED — generated `//spore:` lines are machine-owned; the writer (3c) overwrites on every run.
