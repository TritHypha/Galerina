...
2. **Gap B (Key Custody registry) — v0 SHIPPED 2026-06-17.** `governance/revocations.json` (canonical, append-only; bootstrap `8eecf4187ebc9341`) + `governance/revocation-registry.mjs` (`isKeyRevoked`) + a fail-closed `v(k)` pre-check in `logicn.mjs verify` (`LLN-MANIFEST-REVOKED-KEY`): a manifest signed by a revoked key is **Denied even with a valid signature**, and an unreadable registry fails closed. 4 tests (`tests/revocation/revocation-registry.test.mjs`). **Remaining hardening:** self-sign the registry + verify its own signature before trusting it (trust-anchor pinned); also wire `isKeyRevoked` into `border-check` + `bridge-attestation`; promote the primitive into `logicn-core-security` for suite coverage.
