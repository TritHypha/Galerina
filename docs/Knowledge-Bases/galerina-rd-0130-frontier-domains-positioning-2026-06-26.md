...
| 7 | CHERI VM-free multi-tenancy / "0-cycle enclave" / 60% cost cut | Hardware/Ops | **REFUTED** (CHERI is NOT a compile target — Galerina→WASM; layer-conflation per the CHERI R&D doc; "0-cycle"/"60%" = refused O(1)/beats-silicon). Real shipped thing = SPORE-SUBSTRATE-005 compute-only fence | 1 | REFUTE. Keep the honest compute-only-fence framing; CHERI stays a below-Galerina host-TCB hardening NOTE. |
...
| 12 | Kinetic-AI / cobot safety (AI-proposal vetted against geometric invariants) | Resilience | **RE-DERIVES SHIPPED** (AI-proposal safety + `invariant { ensure … }` SPORE-INV-001..004 + ai.inference governance) | 6 | Buildable **example/demo** (like the gaming-substrate example). Good showcase. |
...
4. **Move/borrow checker** (#18/#19) — emit the RESERVED SPORE-MEMORY-001/002/003/007 (the deferred Rust-parity gap; large).
...
- **Biggest single honest gap surfaced:** memory-safety is **NOT yet Rust-parity** — only affine consume-once is enforced; SPORE-MEMORY-001/002/003/007 (move/borrow/UAF) are reserved-with-no-emitter. The notes are right to flag it; it remains the largest deferred build.
...
- **#3 constant-time lint — SHIPPED** (SPORE-SECRET-004, commit d202c5c). Secret-dependent `if`-branch = timing side-channel; declassifiers (constantTimeEquals/redact) exempt; 0 corpus FP.
...
- **#4 move/borrow checker — large, deferred** (the reserved SPORE-MEMORY-001/002/003/007; the honest Rust-parity gap).
