...
| Secret **taint inference + egress guards** (`SPORE-SECRET-002`) | **core** | Compile-time data-flow proof | shipped (`value-state-checker.ts`) |
| Signature **keygen / verify** (Ed25519; hybrid Ed25519+ML-DSA-65 construction) | **core** | crypto-on-core, bit-exact | shipped; **opt-in hybrid Ed25519+ML-DSA-65 .lmanifest signing SHIPPED (beta.2, `d57699d`/`0f0976c`)** — DEFAULT remains Ed25519 (unchanged); `GALERINA_MANIFEST_PROFILE=certified` MANDATES hybrid, both halves required, fail-closed (`SPORE-MANIFEST-PQ-REQUIRED`/`-PUBKEY-MISSING`/`-TAMPER`). What stays gated is **production key custody + the offline key-generation ceremony** (mandatory-by-default), NOT the signing construction — see §4/§5. |
