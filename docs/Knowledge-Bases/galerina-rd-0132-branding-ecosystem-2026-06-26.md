...
| 2 | **`.spore`** file extension | **RESOLVED 2026-06-26 (owner):** `.spore`→`.spore` (SOURCE); `.tmf` STAYS `.tmf`. branding-3's `.tmf`→`.spore` is rejected. | **SETTLED** — `.spore` = the source extension only; the compiled passport keeps `.tmf`. No longer a rebrand blocker. | 8 |
| 3 | **`.tmf`** = compiled passport (TritMesh File) | shipped (`galerina-ext-tmf`) | KEEP as-is under branding-2; OR rename `.tmf`→`.spore` under branding-3 (then source needs a different ext). Tied to #2. Note: `.tmf` carries ML-DSA-65 sigs — renaming the EXTENSION is cosmetic, but its internal magic/format strings are wire-format (don't rebrand those — same crypto-versioning rule as RD-0131). | 6 |
...
| 6 | **`.spore` as the `.tmf` passport rebrand** (branding-3) | — | Same collision as #2; if `.tmf`→`.spore`, source keeps `.spore` (or another ext). Owner picks. | 5 |
...
## Key findings (R&D)
1. **The `.spore` collision — RESOLVED by owner 2026-06-26.** `.spore`→`.spore` (source); `.tmf` stays `.tmf` (compiled passport unchanged); branding-3's `.tmf`→`.spore` rejected. Naming is now fully settled — Galerina (compiler) · `.spore` (source) · `.tmf` (compiled passport) · TritMesh (network). No longer a rebrand blocker; the only remaining gate on task #67 is owner "go" (timing).
...
- **Verify-before-build** — TritMesh DB shipped-status checked against the tree; the `.spore` collision is a literal contradiction between the two notes, not an inference.
