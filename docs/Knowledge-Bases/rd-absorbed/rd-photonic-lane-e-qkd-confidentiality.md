...
|---|---|---|---|
| **BB84** (1984) + **decoy-state** (Lo–Ma–Chen / Wang 2005) | the workhorse prepare-and-measure protocol; decoy states defeat photon-number-splitting | metro range; **Mbit/s** class commercially (Toshiba T12 = decoy-state BB84, ~tens of km — IDQ's current Cerberis 3/XG line ships QKD but runs **Coherent-One-Way**, *not* BB84) | trusts the detectors |
...
| **Device-independent QKD** (2022) | removes the **device-trust** assumption | **~0.07 bits/event** asymptotic; finite-key block (~1.75×10⁵) **not** reached in a 3,342-round / 75-h run, 400 m (Zhang/van Leent/Redeker, *Nature* **607, 687**). Sibling trapped-ion DI-QKD (Nadlinger et al., *Nature* **607, 682**) *did* complete a finite-key proof (~0.064 bits/event, ~8 h, ~2 m). **Superseded 2026:** DI-QKD to 100 km at **0.112 bits/event** finite-key (Lu et al., *Science* **391, 592**) — still no trusted nodes/repeaters | research-grade; far slower |
...
- **A one-time pad is impractical at bandwidth over real deployment distances.** OTP needs a fresh secret key
  **as long as the message** (§Shannon), and QKD key rates collapse with distance — so no *long-haul* QKD link can
  pad a Gbit/s data stream. *(Honesty caveat: ≥1 Gbit/s secret-key rates **have** been demonstrated at short metro
  reach and/or under weaker/asymptotic security models — floodlight QKD ~1.3 Gbit/s [Quantum Sci. Technol. 3,
  025007, 2018], integrated CV-QKD ~1.21 Gbit/s [arXiv:2504.08298] — so the limit is engineering-practical, not
  categorical.)* OTP stays a niche-of-a-niche (very low-bandwidth, ultra-high-value).
...
- **ETSI TS 103 744** *Quantum-safe Hybrid Key Establishment* (CatKDF/CasKDF over HKDF/RFC 5869) — the QKD-aware
  combiner (title is version-dependent: *Hybrid Key Exchanges* v1.1.1, 2020 → *Hybrid Key Establishment* v1.2.1,
  2025). **ETSI ISG-QKD**; **ITU-T Y.3800** (2019) is the QKD-network *overview* Recommendation — the
  *functional/reference architecture* is **ITU-T Y.3802** (with Y.3801 requirements, Y.3803 key management).
...
- **Where it pays off (topologies):** (a) **data-center / rack interconnect** — short dark fibre, high QKD rate,
  the ideal case; (b) **metro point-to-point** — commercial Toshiba (T12, decoy-state BB84), Mbit/s over ~tens of
  km (IDQ's Cerberis line ships QKD but on Coherent-One-Way, not BB84); (c) **satellite
...
eavesdropping (clean QBER **0%** → accept; intercept-resend QBER **~25%** [24.7–25.2% across runs, ≈ the BB84
intercept-resend expectation] > the 11% threshold → abort); the
...
carry ~1–5% baseline QBER). The **~11% abort threshold is set by the security proof** — the Shor–Preskill bound
`r = 1 − 2·H₂(Q)`, which reaches zero at `Q ≈ 11%` when all observed error is attributed to Eve (PRL 85, 441,
2000) — **not** by the baseline noise; the noise floor only explains why a *practical* threshold can't be literally
0% (channel noise is indistinguishable from eavesdropping). It shows the **principle**
...
| Rates brutal at distance (0.0034 bps @ 1,002 km; kHz satellite) ⇒ **OTP-at-bandwidth impractical over real distances** ⇒ QKD-keyed AEAD, not OTP (Gbit/s SKR exists only at short metro reach / weaker models) | **Grounded, cited (overclaim softened)** |
...
- **ETSI TS 103 744**, *Quantum-safe Hybrid Key Establishment* (CatKDF/CasKDF; title was *Hybrid Key Exchanges* in
  v1.1.1, 2020 → *Hybrid Key Establishment* in v1.2.1, 2025); **ETSI ISG-QKD**; **ITU-T Y.3800** (2019, the QKD
  *overview* Recommendation) — the QKD-network **functional architecture** is **ITU-T Y.3802**.
...
- Liu et al., *Experimental Twin-Field QKD over 1000 km Fiber*, **Phys. Rev. Lett. 130, 210801 (2023)** —
  https://link.aps.org/doi/10.1103/PhysRevLett.130.210801 (1,002 km, no trusted nodes). *Provenance: the **0.0034
  bps** throughput is from CAS/press materials; the PRL reports the rate per-pulse (9.53×10⁻¹² /pulse asymptotic) —
  dimensionally consistent, but the bits/s figure is press-tier, not primary.*
...
- DI-QKD: Zhang, van Leent, Redeker et al., *A device-independent QKD system for distant users*, **Nature 607,
  687–691 (2022)** — the load-bearing **~0.07 bits/event, 3,342-round / 75-h, finite-key-not-reached** result (two
  Rb-87 atoms 400 m apart); Nadlinger et al., **Nature 607, 682–686 (2022)** (trapped-ion DI-QKD, ~0.064 bits/event,
  ~8 h, ~2 m — *did* complete a ~95,884-bit finite key); **superseded** by Lu et al., **Science 391, 592 (2026)**
  (DI-QKD to 100 km, 0.112 bits/event finite-key — still trusted-node-free).
- Shannon, *Communication Theory of Secrecy Systems* (1949) — the OTP key-length bound.
- **NSA** QKD/QC guidance and **NCSC** PQC-over-QKD position (each their own national guidance); the **2024 joint
  *Position Paper on Quantum Key Distribution*** (26 Jan 2024) was authored by **ANSSI (FR), BSI (DE), NL-NLNCSA, and
  Sweden's NCSA** — *not* by the NSA or NCSC. (ANSSI verbatim: QKD is *"extra physical security on top of algorithmic
  cryptography, not a replacement."*)
