...
Two real fail-opens in the `substrate {}` crypto-on-noisy-lane gate (`LLN-SUBSTRATE-001`), both
...
**undetected** — the precise thing `LLN-SUBSTRATE-001` exists to prevent. Both fixed; both pinned
...
rejected by `LLN-CRYPTO-PQ-001`, forcing `crypto.sign.hybrid` / `crypto.sign.mldsa65` /
...
Empirical (pre-fix): `crypto.sign` on photonic → `LLN-SUBSTRATE-001` ✓; `crypto.sign.hybrid` on
photonic → **no `LLN-SUBSTRATE-001`** ✗.
...
early-return and was swallowed: no `LLN-SUBSTRATE-002`, and the crypto-on-noisy gate dropped.
...
`LLN-SUBSTRATE-002` (error) instead of masquerading as an author-chosen inert digital lane. The
diagnostic message + suggestedFix now name the `lane` value set and the trailing-comment gotcha.

Empirical (post-fix): `lane: gaming` → `LLN-SUBSTRATE-002` ✓; `lane: photonic // comment` +
`crypto.sign.hybrid` → `LLN-SUBSTRATE-002` (build fails closed) ✓; clean `lane: photonic` +
`crypto.sign.hybrid` → `LLN-SUBSTRATE-001` ✓.
...
- `substrate-contracts.test.mjs`: **38 pass / 0 fail** (was 30; +8 regression tests covering the
  ...
  lane failing closed).
- `examples/gaming-substrate/` — all three `.lln` verified with `node logicn.mjs check` to match
  their `expected_diagnostics` headers (01 clean, 02 clean, 03 `LLN-SUBSTRATE-001`).

*Source: this session (2026-06-25), full-auto. Both fixes in `logicn-core-compiler/src`
