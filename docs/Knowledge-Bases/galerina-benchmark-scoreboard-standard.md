...
**Owner rule 2026-06-23.** Every benchmark scoreboard presented for Galerina — in chat, docs, or a report —
...
[scoreboard-slowdown feedback](#) and prevents the two ways a Galerina scoreboard lies.
...
3. **Galerina ×slower vs the winner — BOTH tiers, always** (the standing rule): the shipping path **WASM ▶
   production** *and* the diagnostic **governed ⟨interp⟩**. Never present winner/runner-up without the
   Galerina-slowdown column.
...
1. **The diagnostic-tier "win."** The three `⟨interp⟩` rows — **Galerina passive** (LRU-cache warm path),
...
> Related: the standing `feedback-scoreboard-galerina-slowdown-column` (every scoreboard shows ×slower vs winner)
> · `galerina-benchmark-suite` (unit-truth history) · R&D 0092 (tri-logic/data-query workload unification).
