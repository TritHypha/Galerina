...
The earlier [runtime-graph R&D](galerina-rd-graph-secrets-and-runtime-graph-2026-06-25.md) **refused** a *production*
...
- **`galerina-governance-telemetry`** (`exposition.ts` + `server.ts`): `buildGovernanceSnapshot()` already captures
...
- **`galerina-devtools-benchmarks`**: runner + mem-sampler + compare + snapshot + `results/` + cross-language checksum.
...
**Non-production by construction:** gate it with the same `GALERINA_PROFILE` resolver so it *physically cannot attach
