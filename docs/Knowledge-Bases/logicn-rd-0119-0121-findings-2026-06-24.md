...
- **0016 generator** is **partially shipped** (not 0%): `test-generator.ts` + `logicn gen-tests` ship; residual = file-emission (`--out`) + runnable assertions + CI regenerate-and-diff. **OWNER-GREENLIGHT** (MEDIUM feature, not security).
...
- **`RD-0120-F5` circuit_breaker posture-trip is a no-op** (parsed, inert; "DRCM Phase 5"). **BUILD-NOW interim:** make it fail LOUD (`LLN-RES-CB-PENDING`) so a declared-but-inert safety control never reads as enforced.
