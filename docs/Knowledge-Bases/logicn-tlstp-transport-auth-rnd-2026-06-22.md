...
re-verified against live LogicN source/KB (crypto-on-core violations, "already-shipped" overclaims, re-derive
risk), then a synthesis + a completeness critic that re-read both notes and re-checked load-bearing citations in
the repo. The critic's three corrections (LLN-ENTROPY-002 is doc-only not enforced; `authority-model.md` anchor is
...
- **~75-85% of the notes RE-DERIVE already-shipped or already-decided LogicN architecture.** The TLSTP framing
  ("separate the crypto math you must not invent from the transport state machine you may invent") is literally
  LogicN's own **crypto-on-core invariant** (`architecture-charter.md:147-150, 192-196`). Adopt as an organizing
...
|---|---|---|---|
| D1 | **Ternary Ephemeral Ratchet** — fold `E_ternary` physical entropy into the per-packet KDF | **REJECT `E_ternary` in the KDF.** Forward secrecy = digital one-way ratchet + key erasure. If physical entropy is wanted, condition it through SP 800-90B/90A into a Binary CSPRNG, **outside** the cipher. | Hard crypto-on-core violation (`charter:192-196`; `LLN-SUBSTRATE-001` enforced at `substrate-model.ts:257,311` `CRYPTO_ON_NOISY_LANE='error'`). Internally broken: analog is non-reproducible (emulator `deterministic=false`, `ENOB_CEILING=8`) → endpoints derive different keys. Shipped substitutes: SHAKE256 key-erasure chain (`tmf-history-chain-v0.md §2`), hybrid X25519+ML-KEM-768 (`kemdem.ts:143-150`). |
...
5. **Mid-compute capability revocation orchestration** (R&D 0015) — primitives exist (`decideAtBoundary` + `TowerRuntime.evict` + `tpl-simulator.erase`, V_DPM monotonic `LLN-MONO-001`); only the orchestration is unbuilt; strip the float, gate stays K3.
...
- **CONFLICTS/REFUTED (two grounds):** "Ternary Symbol Repair / request only the missing entropy / delta-matrix on the wire" — (a) AEAD/MAC fail closed on a single bit, ciphertext isn't analog-repairable; (b) re-opens the killed cleartext-semantic-routing leak (vec2text ~92%, `LLN-PRIVACY-002`). Only sound residue = digital FEC over opaque ciphertext.
- **REFUTED:** balanced-ternary "spatial/semantic clustering" of `.tmf` via a "tower-discrete-engine" 3D graph — conflates the K3 Kleene lattice with continuous vector geometry (the recurring quantum/continuous category error); "tower-discrete-engine" has zero repo references (invented; real components are `logicn-tower-citizen` + `logicn-ext-photonic-emulator`).
- **REFUTED (framing):** "single-WASM monolith = mathematical security" as settled; "abolish ALL CAs" as a crypto primitive (LogicN governs OVER a library-validated chain via pinned keys — no-CA pinned-key admission IS shipped, but "abolish all CAs" is a deployment stance); physical optical state as an unspoofable auth factor (PAC-learnable).

---

## Crypto-on-core violations (the hard list — keep out of any build prompt)

1. **HARD (refute):** Ternary Ephemeral Ratchet `K_{n+1}=KDF(K_n, E_ternary)` (note 42 Doc003 §4) — analog entropy into the per-packet key. Forbidden (`charter:192-196`; `LLN-SUBSTRATE-001` enforced). Doubly broken (non-reproducible). Lawful: SP 800-90B/90A → Binary CSPRNG outside the cipher.
...
- **Deny-by-default capability model** — `fuse-loader.ts` Gate-3 (`LLN-FUSE-UNKNOWN-CAP`); `capability-types.ts` bit positions, known caps, banned wildcard roots. Defeats bearer-token-possession at the package boundary.
- **Conjunctive K3 admission gate (the real "token tree" primitive)** — `three-valued-governance.ts` allOf/anyOf (empty=INDETERMINATE) + `decideAtBoundary` (INDETERMINATE→deny, `LLN-GOV-3VL-001`). The 4-node tree is presentation over the shipped conjunctive gate.
- **Attested hardware tier (NOT self-asserted)** — `rd-hardware-tier-directive`: resolved once at admission behind `verifyAttestation`; `!attested ⇒ binary floor + K3 DENY` (`LLN-HW-004`). Closes the note's self-asserted hardware fail-open.
...
- **SSRF/metadata-endpoint defense + verify-before-parse** — `egress-guard.ts` (169.254.169.254 deny, URL-credentials SSRF deny); `wasm-runtime.ts verifyWasm` performs NO instantiation. (LogicN admits signed manifests, doesn't parse SAML XML — XSW critique valid-but-moot.)
...
- **Vault key-custody split** — [[logicn-key-custody-rotation-decision]]: rotation executes in `logicn-ext-secrets-vault`, `onRotationFault=halt`, "never serves a stale key."
...
- **Flat SoA AST (DOD)** — 2.22×, Int32Array. (As a DB-engine principle it's moot — the DB engine doesn't exist; `logicn-data-*` are README+package.json only, no `src/`.)
...
- **govern-don't-absorb storage split + `.tmf`-as-passport** — standing invariant (SealTaint `LLN-PRIVACY-002`). **Correction to the note:** the capability GRANT lives in the signed `.lmanifest` `fuse{}` block, NOT the `.tmf` (the `.tmf` is integrity/confidentiality only, no capability grant).
...
- **"tower-continuous-engine" / "tower-discrete-engine"** measuring φ/θ/t — zero repo references (author's coinage); real components are `logicn-tower-citizen` + `logicn-ext-photonic-emulator`.
...
**DO NOT build B8 / `logicn-framework-api-server` — it is owner-locked and these notes ARE the owner's R&D for it.**
...
5. **Morphing default.** Opt-in `transport.obfuscate` (deny-by-default). Frame sizing MUST derive from the digital
   AEAD/KDF stream; a morphed frame must REPLACE any cleartext routing tag (else re-opens `LLN-PRIVACY-002`).
...
6. **Identity on the wire.** Bind the SHIPPED content-addressed signed identity (fuse-loader Gates 1/2/2b +
   ...
   **ECH + Oblivious HTTP (RFC 9458)** pattern; do NOT invent a bespoke LogicN mechanism.
...
- **`LLN-ENTROPY-002` is DECIDED/doc-only, not enforced** — zero `.ts` occurrences (lives in
  `logicn-qrng-entropy-capability-design.md`). The Ratchet refutation (D1) survives on the *enforced*
  `LLN-SUBSTRATE-001` (`substrate-model.ts:257,311`) + non-reproducibility grounds.
