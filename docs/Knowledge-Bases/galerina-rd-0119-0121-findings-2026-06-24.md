...
- **`RD-0120-F5` circuit_breaker posture-trip is a no-op → ✅ FIXED this run** (`a3a827f`). Parsed-but-inert (DRCM Phase 5); now fails LOUD via `SPORE-RES-CB-PENDING` (warning) so a declared-but-inert safety control never reads as enforced. +2 tests, suite 114/114.
