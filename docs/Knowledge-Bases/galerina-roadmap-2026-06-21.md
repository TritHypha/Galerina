...
| **#169 host Char classifiers** | `isUpper/isLower/isWhitespace` wired into the `galerina.mjs` run-host (had drifted behind `createHostRuntime`) | `9df005a` |
...
| **Build/env-secrets doc** | how a Galerina app builds today (one signed `app.wasm` + per-package fusables) + the `.env` trust model folded in | `59ca508` |
...
| **run-host unification** | `galerina run --wasm` broken for string-heavy flows (~11 missing host fns in the inline run-host) | result-reading refactor OR hand-mirroring code-point string fns — user-facing command, needs a "go" |
