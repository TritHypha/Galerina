...
| `logicn-rd-env-tmf-sealed-secrets-2026-06-23.md` | **`env.tmf` sealed-secrets R&D** (wf `wm49burtv`) — the SOPS/Sealed-Secrets/age pattern on the `.tmf` container as an **optional separate pkg `@logicn/ext-secrets-tmf`** (thin layer over logicn-ext-tmf + secrets-vault). Reuses modality=9 Structured + KEM-DEM `seal` + fail-closed `readTmf`; net-new = env.tmf schema · in-memory CRUD CLI (decrypt-in-arena, NO temp file = the SOPS fix) · the §7 encrypted-container reader (specced-unbuilt) · key-custody anchor (the unsolved secret-zero, via the existing `kms`/`vault` SecretConfigSource). Local-edit ≠ production read-back. Defensive-pub. |
