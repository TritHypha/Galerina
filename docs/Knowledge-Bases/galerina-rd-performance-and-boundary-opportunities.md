...
| **"No cleartext semantic embedding across a trust boundary"** (U2 / `SPORE-PRIVACY`) | ✅ use | vec2text recovers ~92% of short text ⇒ sharing embeddings ≈ sharing documents. A genuine egress rule for the boundary; partly shipped (`galerina-privacy-embedding-egress.md`), extensible to a checked diagnostic. |
| **QRNG entropy capability** (Q1 design) | ✅ use (when HW) | Hardens key/nonce generation **at** the boundary (side-channel/fault), fail-closed `entropy.qrng` (`SPORE-ENTROPY-001/002`). Design done; HW-gated. |
| **PEP/PDP governance seam** (TritMesh design-note 02) | ✅ use (pattern) | A clean external-host boundary: enforcement point calls compiled `.spore` PDP across WASM, `unknown→deny`. Canonical reusable architecture (`galerina-tritmesh-boundary-and-seam.md`). |
