...
extend Galerina's governance model, plus the package boundary they must respect. Parts are
...
against Galerina's actual subsystems.
...
> verifiable, signed artifact. The rotation **engine** stays in ext (`galerina-ext-secrets-vault`
...
> `tests/secrets-body-retention-110.test.mjs`. See [[galerina-key-custody-and-rotation]] §2.

## 0. The unifying pattern — governed contract blocks are *dual-mode*

Galerina already has a precedent worth generalizing: `economics {}` can run in **auto** mode
...
> "≥ $1,000") are *illustrative*. In Galerina they should be **derived from the `ValueGraph`
...
+ governance-signature layer (Ed25519 v1 / ML-DSA-65 hybrid, FIPS 204) Galerina already has.
...
**Strict rule: the cloud/vault mechanics live in a non-core tier, never in `galerina-core-*`.**

| Layer | Responsibility |
|---|---|
| `galerina-core-compiler` | Parse `contract { secrets {} / epilogue {} }` grammar; stamp `SecurityTaintExtension` bits; build the `ProofGraph` node. **Pure, deterministic, platform-agnostic — emits WAT/WASM only.** |
| `galerina-core-runtime` | Host-side WASI component interceptor; expose the abstract `wasi:secrets/read` / proof-engine hooks via a narrow **WIT** boundary. No vendor code. |
| `galerina-ext-secrets-aws` (NEW, non-core) | Talks to AWS KMS / Vault / Doppler; catches EventBridge/SNS rotation webhooks; handles auth; drops raw key bytes into the core's open memory slot through the WIT handle. |
...
**Why the boundary (sound, and consistent with Galerina's stated philosophy):**
...
3. **Vendor churn isolation** — cloud APIs change constantly; the core governance plane stays
   stable. (Galerina already establishes this precedent with `galerina-db-{sqlite,postgres,mysql,
   firestore,opensearch}` and `galerina-data-*` as non-core integration tiers — `galerina-ext-*`
...
3. **WIT `secrets-gateway` + `wasi:secrets/read` hooks in `galerina-core-runtime`** — the
   capability boundary; no vendor code.
4. **`galerina-ext-secrets-aws` plugin + ephemeral rotation splicing** — host/runtime + cloud;
...
5. **`zk_snark_receipt` proof engine** — external prover/hardware; pure §Tail until a backend
   ...
   - **Package:** `galerina-ext-proof-snarkjs` (non-core, same tier as `galerina-ext-secrets-vault`).
   - **Interface:** `ProverBackend` plug-in contract so `galerina-core-compiler` never imports snarkjs
   ...
   - **Upgrade path:** replace `zkReceiptStub` field in `EpilogueReceipt` with `zkProof: ZkProof`
     ...
     defined in `proof-graph.ts`. See `docs/Knowledge-Bases/galerina-zk-proof-plan.md` for full spec.
   - **Status:** stub live (produces clearly-labelled PENDING receipt). Real prover = Task #29.

Items 1–2 strengthen Galerina's core differentiator (declared-and-enforced governance, fail
