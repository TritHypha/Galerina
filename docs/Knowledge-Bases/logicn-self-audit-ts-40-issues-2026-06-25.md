...
Owner asked (notes 60 §I non-code + notes 61 code-writing): the LogicN compiler/CLI/runtime is **itself** a
...
> assumed). The LogicN **language** structurally prevents ~12 of these classes for app developers (deny-by-default
> effects, Map-backed prototype-pollution-immune records, the taint-checker, `LLN-SOURCE-ESCAPE-001`, the
...
| 60-3 | Dependency-confusion | partial | yes | All internal deps `file:`; `@logicn` scope unclaimed, `@logicn/core` not `private`; LANG strength LLN-PKG-002 + `fuse-loader` registryCheck | config | 58 |
...
| 60-5 | Malicious lifecycle scripts | partial | yes | 0 first-party hooks; only `argon2 install` (legit gyp); no `.npmrc`/`ignore-scripts`; LANG LLN-PKG-004 deny | config | 60 |
...
| 60-11 | Source-map exposure | partial | n/a | 91/93 `private`; `logicn-devtools-project-graph` ships `.js.map`+`.d.ts.map` | build | 62 |
...
| 61-6 | Enum reverse-map key leak | **partial** *(skeptic override)* | no | `DataClassification` regular enum (`logicn-core-economics/src/index.ts:137`) → reverse map; `risk-calculator.ts:78 DataClassification[profile.classification]` CLI-reachable; input pre-validated ⇒ display-only | build | 60 |
...
| 61-8 | Command injection | partial | no | #174 sink fixed → `spawnSync(...,{shell:false})`; **residual dead `execSync` import** `cli.ts:21`/`logicn.mjs:27` | lint | 86 |
...
4. **[ZT 58 · CONFIG · 60-3]** Claim the `@logicn` npm scope; `"private": true` on `@logicn/core` + `@logicn/devtools-project-graph`; keep internal deps `file:`/`workspace:`; turn `requireCertified` on in prod.
...
7. **[ZT 62 · BUILD · 60-11]** `logicn-devtools-project-graph`: exclude `*.map`/`*.d.ts.map` from the published tarball (`files:` allowlist or drop `sourceMap`/`declarationMap`); add a `npm pack --dry-run` CI lint.
...
11. **[ZT 70 · BUILD · 61-3]** Formalize the `typeof`-guard pattern into a shared `@logicn/schema` `parse(unknown): T | trap`; route all external/network parsers through it.
...
- **[61-8]** Remove the dead `execSync` import (`cli.ts:21`, `logicn.mjs:27`); lint banning `execSync`/`exec`/`shell:true` with a non-literal first arg so the #174 sink can't reappear.
