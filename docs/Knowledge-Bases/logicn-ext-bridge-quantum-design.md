...
`LLN-SUBSTRATE-001` (B1 *crypto-on-core*), reached "for free" by declaring the ffsim
...
**Proposed package:** `@logicn/ext-bridge-quantum` (dir `packages-logicn/logicn-ext-bridge-quantum`)
...
LogicN does **not** reimplement ffsim's mathematics. ffsim is a mature, heavily
...
the way `logicn-ext-bridge-cpp` (BitNet) and `logicn-ext-bridge-bitnet` wrap an
external engine — except ffsim runs **out-of-process** (subprocess), never as an
in-process FFI addon. A governed LogicN `flow` declares a bounded quantum-simulation
...
|---|---|---|---|
| **Governance (Brain)** | trusted | LogicN / TypeScript, in-process | the contract, limit gates, Tower lifecycle, attestation, Toxic Border |
...
| Build it inside `logicn-substrate-mytri` as a "Tri-Bit Quantization Bridge (TBQB)" | `myTri` / `MeshQL` / `TBQB` are a **separate project**, not LogicN. There is no `logicn-substrate-mytri` package in this repo. | This bridge is a **standalone `ext-bridge` package**, peer to `ext-bridge-cpp`. No TBQB, no NTT intrinsics, no MeshQL. |
...
**The legitimate value** of ffsim to LogicN is real and narrow: it is an excellent
*first non-trivial scientific backend* to demonstrate that LogicN's governance model
...
The existing `InferenceBridge` contract (`@logicn/inference-bridge-contract`:
...
- the **Hardened Border** 5-stage cycle + blacklist protocol (`logicn-hardened-border.md`);
...
- **Compiled core:** a Rust extension (`ffsim._lib`) built with **maturin/PyO3**, using **`ndarray` + `rayon`** (multithreading via `RAYON_NUM_THREADS`). From a LogicN/TS perspective this is opaque compiled code we run **out-of-process** — never linked into core.
...
the existing `ai {}` block (see `examples/foundations/ai-inference-governed.lln`).
...
(§12):** the canonical V_DPM bit index must be assigned against `self-hosted/dss/vdpm.lln`
...
(`logicn-substrate-contracts.md`, Direction B; `substrate-model.ts`, Direction C).**
...
- **B1 / `LLN-SUBSTRATE-001` (crypto-on-core)** — a flow on a `noisy` lane may carry
  **no Hash/Sign/crypto effect**, an `error` in *every* profile. This is exactly the
  ...
  produced bytes.)
- **B3 / `LLN-SUBSTRATE-004` (determinism preservation)** — an **un-voted** (`redundancy: 1`)
...
- **B2 / `LLN-SUBSTRATE-002`/`-003` (redundancy sufficiency)** — the declared
...
import type { BridgeManifest, BridgeAttestation } from "@logicn/inference-bridge-contract";
...
ffsim is **Tier 3 untrusted external compute**. The full `logicn-hardened-border.md`
...
Stage 2 — Interrogate   validate the job against schemas/data_types.json (strict): op ∈ enum,
                        ...
                        params are finite numbers only. Any breach → LLN-BORDER-001..004 SECURITY_ALERT,
...
Stage 3 — Execute       spawn the pinned worker (child_process) in a confined scratch dir:
                        ...
                        Unexpected exit (segfault / OOM kill / non-zero) → LLN-BORDER-005 → blacklist version.
...
- **ffsim's Rust stays in ffsim**, in a separate OS process. Nothing from ffsim is
  linked, FFI'd, or `pip install`ed into LogicN core. (Charter: No-Rust-in-core.)
...
- **No crypto on the noisy lane (`LLN-SUBSTRATE-001`, RATIFIED).** An ffsim flow
...
- **No un-voted noisy result into a deterministic sink (`LLN-SUBSTRATE-004`).** An
...
const FFSIM_MANIFEST: BridgeManifest = {
  ...
  packageName:         "@logicn/ext-bridge-quantum",
...
### 9.1 Ratified schema change to `@logicn/inference-bridge-contract/manifest.ts`
...
- The **compute** (ffsim, `lane: noisy`) is tolerance-bounded. By `LLN-SUBSTRATE-001`
...
- No invented crypto: SHA-256 stays (already quantum-OK, Grover→128-bit — see
  `logicn-quantum-resistance-posture.md`); the signature is Ed25519 today, ML-DSA-65
...
all three pins are present, and the consuming flow is `LLN-SUBSTRATE-001`-clean.

---

## 10. Example `.lln` flow (illustrative — does not compile yet)
...
import plugin safe "@logicn/ext-bridge-quantum" as Quantum {
  contract {
...
contract {
  ...
  ;;   B1/LLN-SUBSTRATE-001 — no crypto effect on this flow (integrity stays on the core)
  ;;   B3/LLN-SUBSTRATE-004 — an un-voted (redundancy:1) scalar can't gate a deterministic sink
  substrate {
    lane: noisy
...
Like `ai-inference-governed.lln`, the `quantum {}` block is **not decoration** — it is
...
Mirror `logicn-ext-bridge-cpp` / `logicn-ext-bridge-bitnet`.

### 11.1 Directory layout

```
packages-logicn/logicn-ext-bridge-quantum/
├── package.json
├── package.lln.json                 ← LogicN package manifest (kind/provides/capabilities)
...
```json
{
  "name": "@logicn/ext-bridge-quantum",
  "version": "0.1.0",
  "description": "LogicN governed bridge for IBM ffsim — out-of-process fermionic quantum simulation under contract, Tower audit lifecycle + attestation",
  "license": "Apache-2.0",
  "private": true,
  "type": "module",
  "main": "./dist/index.js",
  ...
  "dependencies": {
    "@logicn/tower-citizen": "file:../logicn-tower-citizen",
    "@logicn/inference-bridge-contract": "file:../logicn-inference-bridge-contract"
  },
  "devDependencies": { "@types/node": "^25.9.1", "typescript": "^5.5.0" },
  "keywords": ["logicn", "ffsim", "quantum", "bridge", "governance", "out-of-process"],
  "note": "Stage A: governance wrapper + out-of-process driver + env-absent stub. Stage B: real subprocess execution + OS sandbox. ffsim source (Apache-2.0): C:/wwwprojects/IBM-FFSIM/ffsim. ffsim's Rust stays in ffsim — never linked into LogicN core."
}
```

### 11.3 `package.lln.json` (mirror logicn-api-protocol-rest)
...
- [ ] Confirm `verifySubstrate` (`substrate-inference.ts` / `substrate-model.ts`) already fires `LLN-SUBSTRATE-001` for a `lane: noisy` flow carrying any crypto effect, and `LLN-SUBSTRATE-004` for an un-voted noisy→deterministic feed. If the `float64`/BLAS+Rayon lane noise profile for B2 is missing, add it (§13.8). **No new crypto-exclusion machinery — reuse the shipped pass.**
- [ ] Extend `@logicn/inference-bridge-contract/manifest.ts`: `DeterminismMode += "tolerance"`, optional `tolerance` / `pinnedEnvHash` / `backendArtifactHash`; update `validateManifestShape` + `canonicalManifestString` (field order!) + tests. *(Touches a neutral package consumed by #137 — re-run its boundary report.)*
...
- [ ] Border tests: oversized subspace, NaN/Inf output, worker non-zero exit → `LLN-BORDER-005` → blacklist.
...
- [ ] `logicn border-check` recognises the package; manifest signing via `logicn bridge attest`.
- [ ] Update SOT runtime status, KB index (done here), task ledger (done here → flip #199 status).
- [ ] `.graph/BOUNDARY.md` for the package (run `@logicn/devtools-package-graph`).
...
1. **Certified-tolerance profile — RATIFIED YES (the gate).** P9 certified mode admits a
   ...
   **fail-closed if any is missing** (§9.1.3). **Hard constraint:** a tolerance-certified
   backend may **never** carry a Hash/Sign/crypto effect — that is `LLN-SUBSTRATE-001`
...
7. **V_DPM bit — RATIFIED.** Do **not** allocate speculatively. Assign the `QuantumSim`
   capability bit in `self-hosted/dss/vdpm.lln` (#85/#91, the `bitfield V_DPM {}` register)
   **when implementation starts**, to avoid bit collisions.

### 13.8 Remaining open item (refinement, not a blocker)

- **B2 lane noise profile for the float64 lane.** `verifySubstrate`'s B2/`LLN-SUBSTRATE-002/003`
...
- ✅ **Integrity stays bit-exact (`LLN-SUBSTRATE-001`)** — the tolerance lane carries no crypto; the receipt signs `sha256(output)` on the deterministic core. No new exclusion machinery — the shipped `verifySubstrate` pass enforces it.
- ✅ **No fictional hardware/perf numbers** — only ffsim's own published figure (64-site Hubbard 256 EiB → 19.3 GiB) is cited, as ffsim's claim; the binomial memory formula is exact; no LogicN perf numbers are asserted.
...
- `docs/Knowledge-Bases/logicn-hardened-border.md` — the Toxic Border 5-stage cycle this design applies.
- `docs/Knowledge-Bases/logicn-governed-inference-tower.md` — the `ai {}` block this design's `quantum {}` mirrors.
- `docs/Knowledge-Bases/capabilities.md` — effects-vs-capabilities model used in §5.
- `packages-logicn/logicn-inference-bridge-contract/` — `BridgeManifest`/`BridgeAttestation` reused (with proposed extensions, §9).
- `packages-logicn/logicn-tower-citizen/src/bridge-attestation.ts` — CF-3/CF-7 attestation (#137/#138).
- `packages-logicn/logicn-ext-bridge-cpp/` · `…-bitnet/` — the in-process bridge pattern this one diverges from (out-of-process, job-oriented).
- `notes/33-IBM-FFSIM.md` — the original eval (errors corrected in §2).
- `docs/Knowledge-Bases/logicn-photonic-tri-substrate-rd-agenda.md` — the tolerance-contract philosophy (§5.4).
- `docs/Knowledge-Bases/logicn-substrate-contracts.md` — the `substrate { lane / tolerance / redundancy }` block + `verifySubstrate` pass this design reuses (B1/B2/B3).
- `docs/Knowledge-Bases/logicn-substrate-failure-model.md` — Direction C noise model + `LLN-SUBSTRATE-001..004` (`substrate-model.ts`).
- `docs/Knowledge-Bases/logicn-quantum-resistance-posture.md` — KEEP-SHA-256 + ML-DSA-65 signature decision behind §9.2's bit-exact integrity.
- `docs/Knowledge-Bases/compiler-diagnostics.md` §LLN-SUBSTRATE — the diagnostic family enforcing the crypto-on-core exclusion.
- **`C:\wwwprojects\LogicN-R-AND-D\FFSM\`** — concrete build-readiness R&D grounding mined from the ffsim source: `ffsim-build-readiness.md` (env/container/determinism/Rust core), `ffsim-op-catalog.md` (op catalog + wire protocol + golden test), `_raw-miner-findings.md` (raw `file:line` cites). **Read before Phase 1/2 implementation.**
