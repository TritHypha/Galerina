...
**Status:** binding conventions (2026-06-16). **Purpose:** keep LogicN ahead of the well-known
language/crypto mistakes, and stop today's pragmatic stopgaps from ossifying into legacy. Written
after a security/PQ cycle (LLN-SECRET-002 fix, LLN-PRIVACY-002, #34 hybrid attestation, LLN-CRYPTO-PQ-001)
flagged a few "decide-now-or-regret-later" items. Cross-refs: [[logicn-quantum-resistance-posture]],
[[logicn-privacy-embedding-egress]], [[logicn-governance-rules]].

## 1. The anti-mistake principles (what we already do — keep doing)

| Other-language mistake | LogicN's standing choice |
|---|---|
...
| Harvest-now-decrypt-later | PQ/hybrid signatures, machine-enforced (LLN-CRYPTO-PQ-001) |
| Downgrade attacks | No silent downgrade; per-surface FIPS-204 domain separation |
| Non-exhaustive `match` | Mandatory wildcard arm |
| C bitwise footguns | Bitwise intentionally absent from `.lln` (engine layer only) |
...
- **Marker-effect pattern (`effects { crypto.sign crypto.sign.hybrid }`) — PROVISIONAL v0.** Chosen to
  enforce LLN-CRYPTO-PQ-001 without parser surgery (dotted effects already parse). A first-class
...
- `LLN-PRIVACY-001` = the declarative `privacy {}` block `deny protected X to Y` clause (parsed, Phase 10C+,
  not yet enforced). `LLN-PRIVACY-002` = the dataflow embedding-egress rule (enforced). **Distinct mechanisms
