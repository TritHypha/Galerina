...
1. **Native governed `json.parse` effect (taint/seal-tracked)** — expose via `CapabilityHost`/`resolveCapabilityEffect` so it's auditable and the result tree carries source taint (ties to SPORE-PRIVACY-002 / SealTaint). Reuses JS `JSON.parse`+`jsObjectToGalerina`, but as a declared effect, not a `pure` stdlib call. **Highest-value JSON fix.** *(L · M)*
