...
| H3 | Crypto/admission | **Hybrid (PQ) signed manifest treated as UNSIGNED by the fuse admission gate** — the PQ upgrade silently turns OFF signature verification at load | high → **re-rated** | add a hybrid branch to `verifyManifestSignature` mirroring logicn.mjs:1426-1486 | **◑ PARTIAL — re-rated on verify-before-build.** Production is NOT insecurely admitting hybrid: `verifyManifestSignature` returns "unsigned" for a hybrid sig and the caller's policy **REFUSES "unsigned" under `requireSignature` (fail-closed)** — so this is a *completeness + visibility* gap, not a live production bypass (in dev `allowUnsigned` it's admitted like any unsigned pkg). Done now (`fuse-loader.ts`): the silent downgrade is now LOUD (`LLN-FUSE-HYBRID-UNVERIFIED` warn) + the stale "placeholder" comment corrected. **Full fix (task #49) — SEAM BUILT 2026-06-26.** Verify-before-build CORRECTION: the manifest's hybrid signature (BOTH halves) is over the SAME RFC-8785 canonical body the Ed25519 path already reconstructs — **no ProofGraph envelope is needed** (the stale comment said otherwise). The app-kernel now exposes `FusePackageOptions.hybridVerifier` — an INJECTED verifier (same lean-TCB pattern as `revocationCheck`; the kernel stays PQ-crypto-dependency-free). When injected, a hybrid manifest is VERIFIED at load: `"verified"`⇒admit, `"invalid"`⇒THROW (`LLN-FUSE-HYBRID-INVALID`, tamper), a throwing verifier⇒`LLN-FUSE-HYBRID-ERROR`, `"unverifiable"`⇒unsigned. When NOT injected, the prior behaviour holds (unsigned + loud warn). +1 test (6 cases incl. exact-canonical-bytes assertion). app-kernel 94/0, full suite 5797/0. **Remaining follow-on:** wire a DEFAULT verifier from the CLI fuse path (mirrors how `revocationCheck` is injected from `governance/revocation-registry.mjs`), using `@noble/post-quantum` ml_dsa65.verify + an ML-DSA public-key file convention (a small design decision). |
