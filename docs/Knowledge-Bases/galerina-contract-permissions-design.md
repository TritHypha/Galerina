...
> Per the non-negotiable rules, do NOT use this syntax in `.spore` examples until the parser + verifier land.
...
`permissions {}` already exists as a **boot/task** block (`AstNodeKind "permissionsBlock"`): `boot.spore` uses it for
...
- **(b) Closed vocabulary.** Any device ref not in `KNOWN_DEVICE_PERMISSIONS` (after alias) → **`SPORE-PERM-001` ERROR**
  (stricter than `SPORE-ACCESS-001`'s warning — a privacy-peripheral typo must hard-fail).
- **(c) No wildcards.** `hardware.*` → **`SPORE-PERM-002` ERROR** (mirrors the `SPORE-CAP-001` network-wildcard ban).
- **(d) AI-cannot-self-grant.** An AI-authored/mutated flow that ADDS a device → **`SPORE-PERM-005`** (requires human
...
5. **`SPORE-PERM-003`**: flow body calls a device API (e.g. `browser.camera.*`) without a matching `permissions{}` grant.
...
- **`camera.read`-as-effect is retired** → `DEVICE_PERMISSION_ALIASES` normalises it and re-routes to require a
  `permissions { hardware.camera }` grant, emitting **`SPORE-PERM-004`** (deprecation). Update
...
| Code | Fault |
|---|---|
| `SPORE-PERM-001` | unknown device ref (closed-vocabulary, ERROR) |
| `SPORE-PERM-002` | wildcard device (`hardware.*`, ERROR) |
| `SPORE-PERM-003` | body calls a device API without a `permissions{}` grant |
| `SPORE-PERM-004` | device named as an effect (`camera.read`) — declare in `permissions{}` (deprecation) |
| `SPORE-PERM-005` | AI-added device permission requires human approval (monotonic-widen guard) |
| `SPORE-PERM-006` | flow's `permissions{}` exceeds the environment `browser{permissions{}}` ceiling |

## 8. Build order (when greenlit)
Grammar + `permissionsDecl` AST → device vocabulary table → governance-verifier (SPORE-PERM-001/002/005/006) →
`V_PERM` lowering + manifest `devicePermissions[]` + CFG-fingerprint fold → runtime device-boundary check + audit →
alias/deprecation (SPORE-PERM-003/004) + doc reconciliation. Each step gets ≥1 MUST-FLAG + ≥1 MUST-PASS fixture per
...
- **Today**, a third-party package declares what it may touch in `effects{}` (`network.outbound`, `ai.inference`,
  ...
  `SPORE-FUSE-UNKNOWN-CAP` → `CRITICAL_SECURITY_VIOLATION`. **Capability is conferred by the importer, not declared by
...
- **The `contract.permissions{}` device clause (this doc) is a FLOW-level clause for PHYSICAL DEVICES**
  ...
  down to the env ceiling. `SPORE-PERM-006` enforces the ⊆-ceiling.

**Where the grant lives — at the IMPORT SITE, not in the package.** Capabilities for a third-party plugin are
conferred **where it is imported** (the main module / `boot.spore`), via an `access { grant … }` block on the
...
([`examples/foundations/hardened-border-plugin.spore`](../../examples/foundations/hardened-border-plugin.spore)):

```spore
;; In the IMPORTING module (main / boot.spore) — THIS is where capabilities are SET:
import plugin safe "./plugins/payment-gateway.spore" as Pay {
  contract {
...
may be granted **only in `boot.spore`** (`SPORE-ASSIMILATE-001`). Toxic-Border protocol: the plugin is a hostile DMZ —
...
import site (clamped ≤ the env ceiling, `SPORE-PERM-006`); the plugin never self-grants. So **`permissions{}` is never
...
> `examples/foundations/hardened-border-plugin.spore` (owner correction 2026-06-23: the grant is at the import site).
