...
47/47·4,346 (SOT) figures. Authoritative source: [logicn-runtime-status-SOT.md](logicn-runtime-status-SOT.md).
...
| Item | Result |
|---|---|
| `.td`→`.lln` migration audit | ✅ complete — **0 legacy `.td` files** (399 `.lln`) |
...
| **#177 graph fix** | ✅ `createFileNode` + `extractLogicnSymbols` (logicn-source nodes) + 3 new pkgs in workspace → `.lln` packages now index (api-protocol-rest 0→35, substrate-math 0→48, ext-bridge-quantum 3→170 refs; 1,312 `.lln` node-paths, was 0) |
...
| External idea mining | ✅ 8 repos → 12 ranked governance ideas ([logicn-external-idea-mining-2026-06-15.md](logicn-external-idea-mining-2026-06-15.md)) |
...
| H6 | #177 graph drops all `.lln` files | ✅ **FIXED this cycle** |
...
| H9 | layer-design doc marks fusion-B2 seam-wiring "DONE+verified" but no seam code exists `logicn-framework-layer-design.md:365` | 🟠 OPEN — doc honesty |
...
- `intelligence-search` benchmark reimplements BM25 inline instead of exercising `@logicn/devtools-intelligence`.
...
| `packages-logicn/logicn-core-compiler/ClaragonwwwLOtest-output.txt` | mangled old-Laragon-path junk | delete (if still present) |
...
| orphan `build/*.wasm` (e.g. `logicn-governance-cost.wasm` 2026-06-03) | several | not used by the runner (compiles fresh) — safe to clear |
| `core-security/src/dss/*.lln` (13 files) | uncompiled scaffold in shipped `src/` | move out of `src/` (gated on #38-41/#76) |
...
- **#177 follow-on:** add the remaining ~34 real packages to `logicn.workspace.json` (tower-citizen, sentinels, most devtools, bridges) — they're absent from the graph (tied to #155 npm-workspaces).
- Cleanup manifest (§4).

### B. User-gated (decisions / trust base)
- **H1** cert-profile in the signed pre-image (changes trust base).
- **#149** key **rotation** + CI secret-scan + force-push the rewritten history (scrub ≠ rotation — the leaked `.env.logicn-signing` must be treated compromised).
...
- `logicn.workspace.json` still omits ~34 real packages (graph under-coverage) — partial fix only.
- `packages-logicn/logicn-devtools-project-graph/` is a **nested git repo** (own `.git`) → untracked by parent; decide: absorb (remove nested `.git`) or make a submodule.
...
2026-06-15 15:45 `build/bench-report.md` stands (harness confirmed compiling the correct fresh `.lln`).
