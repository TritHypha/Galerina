...
**Assurance DAG: UNKNOWN** · root `1686ac8b4ea3c444a00112f27e83f214b3d33ff78f1a80b2d114d77078a92ce0` · non-authorizing.
...
| Type checker / Effect checker | 100% | measured |
...
| Linux adapter preparation | 🟨 | Repository implementation is complete: 10/10 pure facts, a retained-directory GNU Linux x86-64/AArch64 adapter, nine stable injected-refusal classes, hostile namespace-change checks, exact no-replace publication and seven process-termination boundaries. Other ABIs fail closed. Native default/all-feature/release and app-kernel 204/204 are green on Windows. The `134da79...` Ubuntu attempt failed closed before host observation because its sibling SLIDE checkout was absent; it supplies no Linux execution evidence. The 2026-09-12 WSL2 round-two rerun at Galerina `cdfebd5b77bb...` and SLIDE `639329f7bb27...` passes the static candidate, functional 6/6, SLIDE contract/catalog, full SLIDE 1060/1060 and platform observer lanes. The WSL native live suites correctly refuse at `LINUX_HOST_FACTS_INCOMPLETE` / `LINUX_PUBLICATION_HOST_NOT_CANDIDATE` because the host is VMBUS-backed WSL2; this is `VIRTUAL_NON_AUTHORIZING` portability evidence, not Ubuntu Desktop admission. A fresh two-bundle transfer set is ready at `docs/platform-handover/ubuntu-desktop/transfer/wsl-round2-2026-09-12-cdfebd5b77bb/`; direct Ubuntu Desktop evidence, controlled reboot and power loss remain separate sacrificial-host rows |
...
| SLIDE evidence and atlas owner authorities | 🟨 | SLIDE now contains a non-authorizing offline evidence seam: exact canonical preimage construction, owner-supplied Ed25519 + ML-DSA-65 detached signing, admitted public-key verification, mutation refusal and `DEFINED_NOT_ACTIVATED`/K3-zero results. It also contains an opaque atlas owner handle that removes caller minimum-anchor input, binds monotonic advancement to an embedding-owned provider and serializes local writers with a fail-closed sidecar lock. The separate SLIDE authority remains distinct from the Galerina governance key. Owner ceremony/lifecycle receipts, a durable production anchor provider, hosted evidence and named Windows/Linux/macOS restart/rollback/crash/power-loss receipts are still absent, so neither evidence authentication nor production atlas authority is activated |
