...
| [`latency-is-not-work-measured-negatives-defensive-publication-2026-06-25.md`](latency-is-not-work-measured-negatives-defensive-publication-2026-06-25.md) | **defensive-pub** + borderline **measured-negative** | Five reproducible measured-negatives that a parallel/exotic substrate buys latency-depth, **not work** (ML-DSA Amdahl wash · photonic GEMM Θ(N²) · holographic ≠ O(1) · tree-as-tensor · interpreter speed-levers) + the Safe-Floor / reduction-≠-matmul complement. Known-physics/info-theoretic; novelty disclaimed. Result A's external-submission novelty gate (native/SIMD re-measure) stays **OPEN** — internal prior-art only. Bundles RD-0117 + RD-0116. |
| [`authenticated-bytes-equal-executed-bytes-materialise-once-defensive-publication-2026-07-04.md`](authenticated-bytes-equal-executed-bytes-materialise-once-defensive-publication-2026-07-04.md) | **defensive-pub** | **DP-RD-0247** — *authenticated bytes = executed bytes / materialise-once.* Representation-divergence (TOCTOU-on-representation) in signed-graph / ReBAC authorization: the MAC covers one byte-view while runtime traversal reads another (getter / proxy / `__proto__` / non-enumerable). Forged 4× across 4 red-team rounds; fix = verify-and-materialise-once into a frozen snapshot every reader consumes; ships the reusable metamorphic `divergence-probe.mjs`. Novelty disclaimed (CWE-367 · RFC 8785 · parser-differential). |
| [`bluehammer-unsigned-derived-capability-mask-defensive-publication-2026-07-04.md`](bluehammer-unsigned-derived-capability-mask-defensive-publication-2026-07-04.md) | **defensive-pub** | **DP-RD-0233** — *unsigned derived capability-mask as a fault-injection target.* A runtime gate reads an unsigned in-memory `grantedCapabilityMask` derived from a signed token — one bit-flip = silent privilege escalation, **not** covered by the signed token (net-new over dp-rd-0225). Fix = re-derive the mask from the verified token + a `{mask, ~mask}` complement guard. Novelty disclaimed (Rowhammer). |
| [`provenance-integrity-is-not-fidelity-defensive-publication-2026-07-04.md`](provenance-integrity-is-not-fidelity-defensive-publication-2026-07-04.md) | **defensive-pub** | **DP-RD-0129** — *integrity ≠ fidelity.* A valid signature attests authenticity + integrity, **not** that the signed artifact is a faithful/correct representation. Two reproduced signed-artifact fail-opens + an acceptance bar: "sign the equivalence verdict, not the bytes." Novelty disclaimed. |
