...
- **Data / artifact availability:** All artifacts are in-repo and re-runnable at the paths named in §8 (`galerina-core-compiler/tests/*.test.mjs`; `galerina-tower-citizen/src/hybrid-engine.ts`); the source invariant and cross-referenced docs are in the internal engineering KB. No dedicated RD-0129 proof script exists (build deferred) — this is disclosed rather than concealed.
