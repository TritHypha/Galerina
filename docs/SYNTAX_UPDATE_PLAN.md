...
|---|---|---|
| T2.1 | `flip(x)` + `all{}`/`any{}` verdict folds; `and`/`or` Verdict∧Bool overloads | A9: mixed `Verdict and Bool` ⇒ compile error; `not`≠`flip` (Bool-only vs Verdict-only); empty `all{}`/`any{}` ⇒ **INDETERMINATE** never vacuous-allow; A12: `any` feeding a governed allow with tainted operand ⇒ error; machine-checked truth tables |
