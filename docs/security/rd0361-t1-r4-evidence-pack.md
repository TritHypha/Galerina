...
**Assembled:** 2026-07-18 · **Tranche:** T1 (the four sentinel boundary twins) · **State:** ★ **FLIPPED 2026-07-18** on the owner's condition-form nod (*"#143 R4 flip go for it"*). The four T1 twins are now **`authoritative`** — recorded in the authority ledger [`docs/security/rd0361-authoritative-twins.json`](rd0361-authoritative-twins.json) and enforced by `audit-kernel-fungi-twins.mjs` (RED-on-regression + missing-target, self-tested). Immediately before the flip the hash gatherer was re-run and the four sha256 were **unchanged**. Shadow-bake in progress: the `.ts` is retained as a differential shadow (its execution-cutover test keeps asserting WASM===`.ts`); it is deleted only post-bake. If any evidence item below later proves false the flip is void and reverted (remove the twin from the ledger).

> **Provenance note (2026-07-19, reconciles the a9e8e28 drift):** this is the DATED pre-flip evidence assembled 2026-07-18, and the flip is now LIVE — the 4 T1 twins are `authoritative` in the ledger and `node scripts/audit-kernel-fungi-twins.mjs` reports `0 shadow · 25 differential · 4 authoritative`, exit 0. The present-tense phrasings below that read pre-flip — item (a) "0 authoritative (none flipped)", the "twins remain differential" line, the `94/94 · 7,448` soak figure, and the "## The ask" section — describe the state AT ASSESSMENT (the precondition that justified the owner's nod), NOT today. Today: the 4 are authoritative, shadow-bake in progress, the `.ts` retained as a differential shadow. The dated evidence body is preserved verbatim (provenance-preserving); this note is the reconciliation.
