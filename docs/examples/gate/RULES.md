...
Every rule below is **enforced by `tools/gate-check.mjs`** (SPEC v0.6). They exist to make the "50-year mistake" (unversioned syntax +
...
| **R5** | **Unknown ⇒ REJECT** — unknown sections, marks, tags, pragma keys, effect names, sensitivity classes, **cut-set fields** (a `cut(<field>)` token must be a declared PRIVACY/SHAPE field). | There is no silent-skip path; that is how dialects are born. |
...
| **R8** | **Privacy resolves + the cut dominates + DECLARES the field** — `deny <class> <qname> -> <sink>`; qname AND sink must resolve (else vacuous ⇒ REJECT); a sensitive read reaches `[+]` ONLY through a `:cut` that dominates it (rung-2); AND that cut must **DECLARE `<qname>`** in its `cut(<field>...)` set (rung-3, `@version 1.2.0`) — a cut naming the WRONG field REJECTS, an un-annotated privacy cut WARNS. | Vacuous rules; a raw read reaching egress uncut (M1); a green cut over the WRONG field (RD-0340). |
...
## The honest caveats (posture B)
The checker is a **necessary-not-sufficient authoring pre-filter**, not a soundness proof. Two residuals defer to the compile-time
`FUNGI-PRIVACY-002`, each surfaced as a **loud INTERIM warning** — never a silent pass:
- **Topological limit** — a sensitive read reaching a *non-`response.body`* egress with no cut the topology can decide. (You'll see
  this warning on `02-write-transaction.gate` — it is expected for a transactional write.)
- **declaration↔body (rung-4)** — with RD-0340, the checker now BINDS the field↔cut *declaration* to the rule at **authoring** time
  (rung-3: a `cut(<field>)` must name the rule's field; a WRONG field is a hard **REJECT**, not a residual). What remains is proving
  the `.fungi` body actually strips the declared field — an **un-annotated** privacy cut warns and defers that to `FUNGI-PRIVACY-002`.

So a passing `.gate` is a *clean map*; the sound guarantee is the **composite** with the production security gate at fuse.
