...
`logicn border-check` scans only [`../plugins/`](../plugins/). This directory holds
...
`audit.write`) with a placeholder `sourceHash` (`sha256:pending-logicn-promote`).
...
5. **Move** the directory back into `../plugins/` and confirm
   `node logicn.mjs border-check` admits it (`1 admitted, 0 denied`).
