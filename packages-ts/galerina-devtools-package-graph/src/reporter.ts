...
export function writeJson(scopePath: string, graph: PackageGraph): string {
  ...
  writeFileSync(path, renderJson(graph));
  return path;
}

/** Render the exact machine graph bytes without performing I/O. */
export function renderJson(graph: PackageGraph): string {
...
export function writeBoundaryMarkdown(
  ...
  writeFileSync(path, renderBoundaryMarkdown(graph, check));
  return path;
}

/** Render the exact human boundary report without performing I/O. */
export function renderBoundaryMarkdown(
  ...
  return `# Package Boundary Report: ${graph.packageName}
