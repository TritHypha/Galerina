...
async function build(src) {
  const prog = L.parseProgram(src, "b2b.fungi");
...
`;
    ...
    assert.ok(wat.includes("$__fungi_zl"), `a body-less \`${block}\` must still emit the secret-zeroing loop (was a fail-open)`);
  }
});
...
test("B2b: a privacy-marked module emits the secret-zeroing loop; a plain module does NOT", async () => {
  ...
  assert.ok(secret.wat.includes("$__fungi_zl"), "privacy/secrets ⇒ zeroing loop emitted");
  assert.ok(!plain.wat.includes("$__fungi_zl"), "no privacy/secrets ⇒ no zeroing (non-secret modules pay nothing)");
});
...
`;
  ...
  assert.ok(wat.includes("$__fungi_xl"), "a primitive-returning secret leaf must emit the on-EXIT zeroing loop");
...
`;
  const { wat } = await build(src);
  assert.ok(!wat.includes("$__fungi_xl"), "a record/heap return must stay on the lazy path (can't zero the returned object)");
});
