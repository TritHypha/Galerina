...
test("example (a): valid pinned chain + fresh OCSP → +1 → ALLOW", () => {
  ...
  assert.equal(decision.diagnostic.kind, "NONE");
});
...
test("example (c): hash-pin mismatch → −1 → DENY, no diagnostic (library-valid cert is not enough)", () => {
  ...
  assert.equal(decision.decision, "deny");
  assert.equal(decision.authorized, false);
  assert.equal(decision.diagnostic.kind, "NONE"); // definite −1 is an ordinary policy denial, not a collapsed 0
});
...
test('revocation "revoked" → −1 → deny, no diagnostic', () => {
  ...
  const decision = certGate(input);
  assert.equal(decision.authorized, false);
  assert.equal(decision.diagnostic.kind, "NONE"); // definite −1, not a collapsed 0
});
