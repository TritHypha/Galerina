...
The **Tri-Pipe topology** for LogicN: a cached, **attested** `hardware()` capability directive
...
> AXIS-2 (the 0053 `route()` in `@logicn/ext-photonic-emulator`) decides, **per kernel**, whether to
...
> DENY → binary, `LLN-HW-004`). The directive is derived from the **attested** `manifest.hardwareIdentity`
...
hardware() :: () -> 'binary' | 'hybrid' | 'photonic'         // cached, deployment-stable
  ...
  3. profiles.get(targetId)           → undefined ⇒ 'binary'  (UNKNOWN ⇒ K3 DENY, LLN-HW-004)
...
code. Composes `@logicn/ext-photonic-emulator`'s `route()`/cost model for the AXIS-2 sweep.
