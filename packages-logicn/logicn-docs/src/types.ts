 ...
 *
 * `x-logicn-*` vendor extensions are permitted on the objects that carry governance
 ...
 * `EffectivePosture`) are owned by `@logicn/framework-app-kernel`; this package
...
// from `@logicn/docs` without reaching into the kernel package directly.
...
export interface SchemaObject {
  ...
  /** `x-logicn-*` provenance extensions only. */
...
export interface OperationObject {
  ...
  /** `x-logicn-*` provenance extensions only (handler, limits, scopes, relaxations). */
  readonly [extension: `x-${string}`]: unknown;
}
