/**
 * Generation tests for @logicn/docs.
 *
...
test("scoped route → 403 + x-logicn-scopes + scopes in description", () => {
  const doc = generateOpenApi({
    info: INFO,
  ...
  assert.deepEqual(op["x-logicn-scopes"], ["admin.read"]);
...
test("body-bearing route → requestBody + 413/415/422 + x-logicn-max-body-bytes", () => {
  const doc = generateOpenApi({
    info: INFO,
  ...
  assert.equal(op["x-logicn-max-body-bytes"], 256 * 1024);
});
...
test("posture 'on' tightens the documented body ceiling to 64 KiB", () => {
  ...
  assert.equal(doc.paths["/secure-upload"].post["x-logicn-max-body-bytes"], 64 * 1024);
});
...
test("relaxations are surfaced (public auth on an otherwise-default route)", () => {
  ...
  assert.deepEqual(doc.paths["/relaxed"].get["x-logicn-relaxations"], ["auth:public"]);
});
