...
The consolidated **LogicN test harness** — one named, consumable package that
...
| **e2e** | `node logicn.mjs check` / `build` | Example apps compile end-to-end through the real toolchain (parse → govern → emit → manifest). |
| **conformance** | `tests/r6-corpus/r6-parity.test.mjs` | The R6 bootstrap corpus passes the Stage-A ≡ Stage-B parity gate. |
| **fidelity** | `logicn-core-compiler/tests/fidelity-differential.test.mjs` | The 0014 differential: tree-walker ≡ bytecode-VM ≡ WASM, byte-exact. |
...
```ts
import { runUnit, runE2e, runConformance, runFidelity, runAll } from "@logicn/test";

const res = await runAll({ core: true });   // CheckResult { kind:"all", ok, exitCode, children, … }
if (!res.ok) process.exit(res.exitCode);
```

Every runner accepts a `rootDir` (the LogicN workspace to target) and a
`timeoutMs`. When `rootDir` is omitted it is auto-detected by locating
`logicn.workspace.json` (from `$LOGICN_ROOT`, then cwd, then this module). A
...
src/cli.ts       the `logicn-test` front door
