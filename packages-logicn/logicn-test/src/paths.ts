...
//
// The harness drives tooling that lives in a LogicN workspace — scripts/, the R6
...
// logicn.workspace.json, so it works whether invoked from the package, the repo
...
/** The file whose presence marks a LogicN workspace root. */
export const WORKSPACE_MARKER = "logicn.workspace.json";
...
/**
 * Resolve the LogicN workspace root.
 *
 * Order of precedence:
 *   1. an explicit `rootDir` (trusted as-is — a consumer points at their workspace);
 *   2. $LOGICN_ROOT;
 *   3. the current working directory, walking up to a logicn.workspace.json;
 *   4. this module's location, walking up to a logicn.workspace.json.
 *
...
export function resolveRoot(rootDir?: string): string {
  ...
  const fromEnv = process.env.LOGICN_ROOT;
  ...
  throw new Error(
    `logicn-test: could not locate a LogicN workspace (no ${WORKSPACE_MARKER} ` +
      `found from rootDir / $LOGICN_ROOT / cwd). Pass { rootDir } or set $LOGICN_ROOT.`,
  );
}
