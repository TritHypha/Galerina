// @logicn/test — shared vocabulary for the consolidated LogicN test harness.
//
// One type model for the four check kinds a downstream app runs against a LogicN
...
export interface HarnessOptions {
  /**
   * The LogicN workspace root (the directory holding logicn.workspace.json). When
   * omitted it is auto-detected: $LOGICN_ROOT, then cwd (walking up), then this
...
/** `e2e` — end-to-end compile of example apps through the shipped `logicn` CLI. */
export interface E2eOptions extends HarnessOptions {
  /** Example entry flows to compile (absolute, or relative to rootDir). */
  readonly examples?: readonly string[];
  /** Also run `logicn build` (not just `check`) on each entry. Default false. */
