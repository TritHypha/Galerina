// Foundation smoke tests for @logicn/test — the harness primitives.
//
...
const ROOT = resolve(here, "..", "..", ".."); // packages-logicn/logicn-test/tests → repo root
...
test("resolveRoot: throws fail-closed when no workspace exists above the dir", () => {
  // A path with no logicn.workspace.json anywhere above it. We point cwd-walk and
  ...
  assert.equal(WORKSPACE_MARKER, "logicn.workspace.json");
});
...
test("resolveTarget: relative paths resolve under the root, absolute pass through", () => {
  ...
  const abs = resolve(ROOT, "logicn.mjs");
