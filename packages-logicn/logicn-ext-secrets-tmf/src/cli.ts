#!/usr/bin/env node
// cli.ts — the `logicn secrets-tmf` CRUD/shell CLI (design doc Part 2).
//
// bin: logicn-secrets-tmf  (also surfaced as `logicn secrets-tmf <cmd>`)
...
function rejectValueInArgv(a: Args): void {
  if (a.rest.length > 0) {
    die(
      ...
      "(argv leaks via ps/proc/cmdline/history). Pipe the value: `printf %s \"$V\" | logicn-secrets-tmf set NAME`.",
    );
  }
}
...
function getWrappedKey(): WrappedKey {
  // The wrapped key is provided out-of-band via env LOGICN_ENVTMF_WRAP (hex of salt|iv|ct) — a
  // POINTER to anchored material, NOT a plaintext secret. We never read a plaintext key from argv.
  const hex = process.env.LOGICN_ENVTMF_WRAP;
  if (hex === undefined) die("set LOGICN_ENVTMF_WRAP to the wrapped recipient key (salt|iv|ct hex)");
      ...
      try {
        ...
        if (process.stderr.isTTY && !a.force) {
          die(
            ...
            "(e.g. `logicn-secrets-tmf keygen 2>key.sec` then wrap+wipe), or pass --force to override.",
          );
        }
