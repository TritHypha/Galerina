...
OPTIONAL, SEPARATE (`private:true`, `ext` tier, not auto-loaded) sealed-secrets layer for LogicN.

`env.tmf` is an **encrypted-at-rest replacement for a plaintext `.env`**, edited through a governed
**in-memory-only** CLI. It is a THIN orchestration layer over the shipped `@logicn/ext-tmf`
(format/crypto) and the `@logicn/ext-secrets-vault` store **discipline**. It adds **NO new crypto**
and **NO new container bytes**; crypto stays Binary (LLN-SUBSTRATE-001).
...
```
logicn-secrets-tmf <cmd> [--file ./env.tmf]
...
```sh
printf %s "$DB_PASSWORD" | logicn-secrets-tmf set DB_PASSWORD --pub "$PUB"
```

Get a value for a process (local-only, piped — refuses to echo to a TTY):

```sh
logicn-secrets-tmf get DB_PASSWORD | my-app --stdin-secret
```

The recipient secret key is supplied to the CLI as a **passphrase-wrapped** blob via the
`LOGICN_ENVTMF_WRAP` env var (a *pointer* to anchored material, hex of `salt|iv|ct`), unlocked by a
...
**Signed root is DEFERRED.** ML-DSA-65 over the TMX root is `@logicn/ext-tmf` slice 4 / #7
...
- **Prod:** anchor through the EXISTING core-config `SecretConfigSource` kinds —
  `{kind:"kms",...}` / `{kind:"vault",...}` (`logicn-core-config/src/index.ts:1144-1145`) via a
...
`@logicn/ext-tmf` is resolved during in-staging build via a `paths` alias (tsconfig) and a local
`node_modules/@logicn/ext-tmf` junction to the shipped package. Once the hub absorbs this package
into `packages-logicn/`, the workspace resolves it natively.
