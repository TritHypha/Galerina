...
test("example (b): revocation responder unreachable → 0 → DENY + LLN-GOV-3VL-001 (the soft-fail hole, closed)", () => {
  ...
  assert.equal(decision.diagnostic?.code, "LLN-GOV-3VL-001");
  assert.equal(diags.length, 1);
  assert.equal(diags[0].code, "LLN-GOV-3VL-001");
});
...
test("single-factor-unknown: each factor = 0 (others +1) → deny + LLN-GOV-3VL-001", () => {
  ...
  for (const zeroed of factors) {
    ...
    assert.equal(decision.diagnostic?.code, "LLN-GOV-3VL-001", `${zeroed}=0 must audit the collapse`);
  }
});
...
test("side-signals fold into certGate: a DENY side-signal sinks an otherwise-ALLOW channel", () => {
  ...
  assert.equal(degraded.diagnostic?.code, "LLN-GOV-3VL-001");
