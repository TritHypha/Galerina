...
// with ZERO LLN-VALUESTATE diagnostics — exactly the omission 0093 fixed for guardedFlowDecl.
...
test("FAIL-OPEN CLOSED: a governed-flow bare param at a governed sink fires LLN-VALUESTATE-008 (was silent)", () => {
...
`);
  ...
  assert.ok(has(r, "LLN-VALUESTATE-003") || has(r, "LLN-VALUESTATE-008"), `governed tainted param must be flagged; got ${r.diagnostics.map((x) => x.code).join(",") || "none"}`);
});
...
`);
  assert.ok(!has(r, "LLN-VALUESTATE-003"), "no taint sink → no VS-003");
});
