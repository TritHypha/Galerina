...
//   LLN-AFFINE-001   — a passport consumed at an authority sink cannot be re-used (affine/linear).
//   LLN-PASSPORT-002 — a passport below a sink's required stage is denied (illegal stage-skip).
// Lifts the rd-0087 proven abstract invariant onto real .lln source. Binary/digital, deny-by-default
...
describe("RD-0111 passport typestate: the two denies fire", () => {
  it("consume-once: a passport re-used at a second authority sink → LLN-AFFINE-001", () => {
  ...
  response.body(a)
    ...
    assert.ok(c.includes("LLN-AFFINE-001"), `expected LLN-AFFINE-001, got: ${c.join(",")}`);
  });

  it("state-skip: a Raw passport at an Authorized-requiring sink → LLN-PASSPORT-002", () => {
    const c = codes(`secure flow skipStages(p: Passport) -> Bool {
  response.body(p)
}`);
    assert.ok(c.includes("LLN-PASSPORT-002"), `expected LLN-PASSPORT-002, got: ${c.join(",")}`);
  });
});
