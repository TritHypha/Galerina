/**
 * @logicn/devtools-provenance — 0098-prov-parsefail fail-closed regression tests.
 *
 * Proves:
 ...
 *  (no regression) a gated flow and the canonical verifyPassword.lln stay clean.
 */
...
const VERIFY_PASSWORD_PATH = resolve(__dirname, "../../../examples/auth-service/verifyPassword.lln");
...
const BROKEN_SRC = "@@@ this is not valid logicn %%% {{{ unterminated";
...
describe("0098-prov-parsefail Fix B: blind analysis denies by default", () => {
  it("a broken source sets analyzerBlind=true", () => {
    const result = analyzeFile(BROKEN_SRC, "broken.lln");
  ...
  it("buildProvenanceGraph counts a blind file toward flowsWithUngatedSinks (CLI exit 2)", () => {
    ...
    assert.ok(graph.riskFlows.some(r => r.flowName === "<parse-failure>"),
      "a blind file must produce an explicit LLN-PROV-001 <parse-failure> risk entry");
  });
});
...
describe("0098-prov-parsefail: clean inputs still pass (no false positive)", () => {
  ...
  it("verifyPassword.lln (canonical clean) has 0 ungated sinks and is not blind", () => {
    ...
    assert.equal(graph.summary.flowsWithUngatedSinks, 0,
      "verifyPassword.lln must remain the canonical clean example (0 ungated sinks)");
  });
});
