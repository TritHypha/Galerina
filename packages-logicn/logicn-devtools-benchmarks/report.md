...
|---|---|---|---|---|---|---|
| **compute-mix** | **LogicN passive ⟨interp⟩** 🖥️ | **850.82M/s** | 215.2K/s | 0.00025× (4.0K× slower) | ❌ 0.22× (4.6× slower) | LRU cache warm path (first-call winner: Node.js at 134.27M/s) |
...
| **fibonacci-recursive** | **LogicN passive ⟨interp⟩** 🖥️ | **83.2K/s** | 11.0/s | 0.00013× (7.6K× slower) | ✅ **1.36×** faster | LRU cache warm path (first-call winner: WASM ▶ production at 45.3K/s) |
...
| **gpu-compute** | **LogicN passive ⟨interp⟩** 🖥️ | **8.35B/s** | 371.8K/s | 0.00004× (22.4K× slower) | ❌ 0.03× (33.7× slower) | LRU cache warm path (first-call winner: WASM ▶ production at 1.56B/s) |
| **matrix-multiply** | ⚠️ not unit-aligned | — | — | — | — | excluded — LogicN/WASM n=32 (1024 cells, 32³ mul-adds), node/python/rust n=64 (4096 cells, 64³), Deno n=128 (16384 cells). Per-cell work (dot-product length n) also differs, so no shared unit is apples-to-apples until n is unified. |
| **crypto-ops** | **LogicN passive ⟨interp⟩** 🖥️ | **19.1K/s** | 274.0/s | 0.0143× (70× slower) | n/a (no Python) | LRU cache warm path (first-call winner: LogicN manifest ⟨interp⟩ at 2.0K/s) |
| **text-html** | **LogicN passive ⟨interp⟩** 🖥️ | **75.7K/s** | 1.0K/s | 0.0132× (76× slower) | n/a (no Python) | LRU cache warm path (first-call winner: LogicN manifest ⟨interp⟩ at 3.2K/s) |
| **tri-logic** | ⚠️ not unit-aligned | — | — | — | — | excluded — LogicN main()=runBulkTri(100000) (100k triples / 300k trit-ops) |
| **data-query** | ⚠️ not unit-aligned | — | — | — | — | excluded — LogicN main()=filterAndCount(1000)+groupByCategory(1000)=2000 record-scans (but logicnOpsPerRun=1000 undercounts) |
| **call-chain** | **LogicN passive ⟨interp⟩** 🖥️ | **4.99B/s** | 60.8K/s | 0.00001× (82.1K× slower) | ❌ 0.03× (34.4× slower) | LRU cache warm path (first-call winner: Node.js at 307.73M/s) |
| **nbody** | **LogicN passive ⟨interp⟩** 🖥️ | **2.24B/s** | 58.5K/s | 0.00003× (38.3K× slower) | ❌ 0.04× (25.4× slower) | LRU cache warm path (first-call winner: Node.js at 123.09M/s) |
| **json-parse** | **LogicN passive ⟨interp⟩** 🖥️ | **47.30M/s** | 7.4K/s | 0.00016× (6.4K× slower) | ❌ 0.01× (86.3× slower) | LRU cache warm path (first-call winner: Node.js at 3.32M/s) |
| **mandelbrot** | **LogicN passive ⟨interp⟩** 🖥️ | **1.20B/s** | 7.9K/s | 0.00001× (150.6K× slower) | ❌ 0.03× (31.8× slower) | LRU cache warm path (first-call winner: Rust (generic) at 23.39M/s) |
| **spectral-norm** | **Rust (generic)** 🖥️ | **371.32M/s** | — | — | — | Native compiled — LLVM optimised, may auto-vectorise |
| **binary-trees** | **LogicN passive ⟨interp⟩** 🖥️ | **10.90B/s** | 3.75M/s | 0.00034× (2.9K× slower) | ❌ 0.70× (1.4× slower) | LRU cache warm path (first-call winner: Node.js at 75.95M/s) |
...
> **Python floor check:** LogicN (governed) beats Python on **1/14** unit-aligned benchmarks where both ran. Python is the like-for-like floor while the Stage-A runtime is still TypeScript-interpreted; Rust/Zig/WASM are the ceiling. (3 benchmark(s) excluded — not unit-aligned; see §1.6.)
...
| matrix-multiply | ⚠️ excluded | cells/s | workload SIZE differs by runtime — LogicN/WASM n=32 (1024 cells, 32³ mul-adds), node/python/rust n=64 (4096 cells, 64³), Deno n=128 (16384 cells). Per-cell work (dot-product length n) also differs, so no shared unit is apples-to-apples until n is unified. |
...
| tri-logic | ⚠️ excluded | trit-ops/s | incomparable workloads — LogicN main()=runBulkTri(100000) (100k triples / 300k trit-ops); node/python/rust run nested 9-element truth-table micro-benches plus a separate 10M bulk; logicnOpsPerRun=27000 (≈27 truth-table combos ×1000) corresponds to none of these. Needs a common bulk-N trit-op path on every runtime. |
| data-query | ⚠️ excluded | records/s | incomparable — LogicN main()=filterAndCount(1000)+groupByCategory(1000)=2000 record-scans (but logicnOpsPerRun=1000 undercounts); node/python run 7 separate query micro-benches nested under results.* with no single representative. Needs a main() recount + a chosen representative query before the numbers compare. |
...
> workloads are realigned across runtimes. Excluding them is what stops false "LogicN wins" on
...
|---|---|---|---|---|---|---|
| 🥇 | ⚪ | LogicN passive ⟨interp⟩ | -70.60 bytes/op ⚡ ~0 — no boxing | 508.00M/s | — | -706KB |
...
| 7 | ⚫ | LogicN governed ⟨interp⟩ | 66 bytes/op ⚠ moderate | 122.1K/s | — | 656KB |
| 8 | ⚫ | LogicN manifest ⟨interp⟩ | 87 bytes/op ⚠ moderate | 115.7K/s | — | 869KB |
...
| compute-mix | LogicN passive ⟨interp⟩ | 76.8MB | 76.8MB | 18.1MB | 20KB |
| compute-mix | LogicN manifest ⟨interp⟩ | 76.8MB | 76.8MB | 16.0MB | 926KB |
| compute-mix | LogicN governed ⟨interp⟩ | 76.6MB | 76.6MB | 16.4MB | 1.6MB |
...
| arithmetic-threshold | LogicN passive ⟨interp⟩ | 77.5MB | 77.5MB | 15.8MB | 9KB |
| arithmetic-threshold | LogicN manifest ⟨interp⟩ | 77.5MB | 77.5MB | 16.9MB | 1.6MB |
| arithmetic-threshold | LogicN governed ⟨interp⟩ | 77.5MB | 77.5MB | 15.8MB | 214KB |
...
| six-digit-guess | LogicN passive ⟨interp⟩ | 79.5MB | 79.5MB | 17.0MB | 37KB |
| six-digit-guess | LogicN manifest ⟨interp⟩ | 79.3MB | 79.3MB | 16.2MB | 548KB |
| six-digit-guess | LogicN governed ⟨interp⟩ | 79.0MB | 79.0MB | 16.3MB | 784KB |
...
| record-allocation | LogicN passive ⟨interp⟩ | 79.9MB | 79.9MB | 16.7MB | 57KB |
| record-allocation | LogicN manifest ⟨interp⟩ | 79.2MB | 79.2MB | 16.5MB | 710KB |
| record-allocation | LogicN governed ⟨interp⟩ | 79.9MB | 79.9MB | 16.6MB | 716KB |
...
| fibonacci-recursive | LogicN passive ⟨interp⟩ | 80.0MB | 80.0MB | 16.5MB | 32KB |
| fibonacci-recursive | LogicN manifest ⟨interp⟩ | 79.5MB | 79.5MB | 17.3MB | 1.4MB |
| fibonacci-recursive | LogicN governed ⟨interp⟩ | 79.8MB | 79.8MB | 17.0MB | 1.1MB |
...
| collection-pipeline | LogicN passive ⟨interp⟩ | 79.4MB | 79.4MB | 16.7MB | 86KB |
| collection-pipeline | LogicN manifest ⟨interp⟩ | 79.3MB | 79.3MB | 16.1MB | 108KB |
| collection-pipeline | LogicN governed ⟨interp⟩ | 80.3MB | 80.3MB | 16.1MB | 93KB |
...
| governance-cost | LogicN passive ⟨interp⟩ | 81.3MB | 81.3MB | 17.2MB | 833KB |
| governance-cost | LogicN manifest ⟨interp⟩ | 82.8MB | 82.8MB | 16.5MB | 445KB |
| governance-cost | LogicN governed ⟨interp⟩ | 81.0MB | 81.0MB | 16.6MB | 463KB |
...
| hardware-targets | LogicN passive ⟨interp⟩ | 80.5MB | 80.5MB | 16.7MB | 50KB |
| hardware-targets | LogicN manifest ⟨interp⟩ | 80.3MB | 80.3MB | 16.2MB | 36KB |
| hardware-targets | LogicN governed ⟨interp⟩ | 80.0MB | 80.0MB | 16.2MB | 36KB |
...
| low-memory | LogicN passive ⟨interp⟩ | 80.7MB | 80.7MB | 16.7MB | -706KB |
| low-memory | LogicN manifest ⟨interp⟩ | 80.4MB | 80.4MB | 17.1MB | 869KB |
| low-memory | LogicN governed ⟨interp⟩ | 80.5MB | 80.5MB | 16.9MB | 656KB |
...
| gpu-compute | LogicN passive ⟨interp⟩ | 80.6MB | 80.6MB | 16.6MB | 101KB |
| gpu-compute | LogicN manifest ⟨interp⟩ | 80.6MB | 80.6MB | 16.7MB | 411KB |
| gpu-compute | LogicN governed ⟨interp⟩ | 80.3MB | 80.3MB | 16.5MB | 202KB |
...
| matrix-multiply | LogicN passive ⟨interp⟩ | 81.2MB | 81.2MB | 17.0MB | 84KB |
| matrix-multiply | LogicN manifest ⟨interp⟩ | 81.2MB | 81.2MB | 17.1MB | 770KB |
| matrix-multiply | LogicN governed ⟨interp⟩ | 80.2MB | 80.2MB | 17.1MB | 738KB |
...
| crypto-ops | LogicN passive ⟨interp⟩ | 81.8MB | 81.8MB | 17.4MB | 550KB |
| crypto-ops | LogicN manifest ⟨interp⟩ | 81.4MB | 81.4MB | 16.5MB | 85KB |
| crypto-ops | LogicN governed ⟨interp⟩ | 81.4MB | 81.4MB | 16.5MB | 209KB |
| text-html | Rust AVX2 | — | — | — | — |
| text-html | Rust (generic) | — | — | — | — |
| text-html | Node.js | — | — | — | 470KB |
| text-html | Python | — | — | 208B | 208B |
| text-html | LogicN passive ⟨interp⟩ | 80.8MB | 80.8MB | 17.1MB | -367KB |
| text-html | LogicN manifest ⟨interp⟩ | 82.0MB | 82.0MB | 16.8MB | 116KB |
| text-html | LogicN governed ⟨interp⟩ | 82.0MB | 82.0MB | 16.9MB | 133KB |
| tri-logic | Rust AVX2 | — | — | — | — |
| tri-logic | Rust (generic) | — | — | — | — |
| tri-logic | Node.js | — | — | — | 1.0MB |
| tri-logic | Python | — | — | 360B | 360B |
| tri-logic | LogicN passive ⟨interp⟩ | 81.2MB | 81.2MB | 17.3MB | -1.7MB |
| tri-logic | LogicN manifest ⟨interp⟩ | 81.2MB | 81.2MB | 18.9MB | 2.0MB |
| tri-logic | LogicN governed ⟨interp⟩ | 80.7MB | 80.7MB | 19.0MB | 2.2MB |
| tri-logic | WASM ▶ production | 80.6MB | 80.6MB | 17.6MB | 2KB |
| data-query | Node.js | — | — | — | 168KB |
| data-query | Python | — | — | 312B | 312B |
| data-query | LogicN passive ⟨interp⟩ | 83.2MB | 83.2MB | 17.4MB | 233KB |
| data-query | LogicN manifest ⟨interp⟩ | 82.5MB | 82.5MB | 20.8MB | 4.0MB |
| data-query | LogicN governed ⟨interp⟩ | 82.2MB | 82.2MB | 20.8MB | 4.0MB |
| call-chain | Node.js | 46.9MB | 46.9MB | 4.1MB | 11KB |
| call-chain | Python | — | — | 368B | 368B |
| call-chain | LogicN passive ⟨interp⟩ | 92.8MB | 92.8MB | 22.7MB | 18KB |
| call-chain | LogicN manifest ⟨interp⟩ | 92.8MB | 92.8MB | 17.9MB | 998KB |
| call-chain | LogicN governed ⟨interp⟩ | 83.2MB | 83.2MB | 18.1MB | 1.2MB |
| nbody | Node.js | 48.3MB | 48.3MB | 4.2MB | 30KB |
| nbody | Python | — | — | 624B | 624B |
| nbody | LogicN passive ⟨interp⟩ | 93.2MB | 93.2MB | 18.1MB | 33KB |
| nbody | LogicN manifest ⟨interp⟩ | 93.2MB | 93.2MB | 18.1MB | 1.2MB |
| nbody | LogicN governed ⟨interp⟩ | 91.1MB | 91.1MB | 17.2MB | 274KB |
| json-parse | Node.js | — | — | — | 252KB |
| json-parse | Python | — | — | 520B | 520B |
| json-parse | LogicN passive ⟨interp⟩ | 92.9MB | 92.9MB | 21.5MB | 80KB |
| json-parse | LogicN manifest ⟨interp⟩ | 92.7MB | 92.7MB | 19.2MB | 1.9MB |
| json-parse | LogicN governed ⟨interp⟩ | 99.0MB | 99.0MB | 20.2MB | 3.3MB |
| mandelbrot | Rust (generic) | — | — | — | — |
| mandelbrot | Node.js | 48.2MB | 48.2MB | 4.8MB | 736KB |
| mandelbrot | Python | — | — | 3KB | 3KB |
| mandelbrot | LogicN passive ⟨interp⟩ | 94.4MB | 94.4MB | 20.6MB | 27KB |
| mandelbrot | LogicN manifest ⟨interp⟩ | 94.4MB | 94.4MB | 20.6MB | 3.4MB |
| mandelbrot | LogicN governed ⟨interp⟩ | 93.9MB | 93.9MB | 18.7MB | 1.4MB |
...
| binary-trees | LogicN passive ⟨interp⟩ | 93.7MB | 93.7MB | 17.6MB | 14KB |
| binary-trees | LogicN manifest ⟨interp⟩ | 93.7MB | 93.7MB | 18.3MB | 1.1MB |
| binary-trees | LogicN governed ⟨interp⟩ | 93.7MB | 93.7MB | 19.3MB | 2.1MB |
...
| compute-mix | LogicN manifest ⟨interp⟩ | 221.7ms | 234.0ms | 106% | 213.68 ops/CPU-ms |
| compute-mix | LogicN governed ⟨interp⟩ | 232.3ms | 235.0ms | 101% | 212.77 ops/CPU-ms |
...
| arithmetic-threshold | LogicN manifest ⟨interp⟩ | 74.2ms | 125.0ms | 169% | 505.98 ops/CPU-ms |
| arithmetic-threshold | LogicN governed ⟨interp⟩ | 86.4ms | 78.0ms | 90% | 810.87 ops/CPU-ms |
...
| six-digit-guess | LogicN passive ⟨interp⟩ | 0.1ms | 0.0ms | 0% | — |
| six-digit-guess | LogicN manifest ⟨interp⟩ | 889.6ms | 953.0ms | 107% | 44.14 ops/CPU-ms |
| six-digit-guess | LogicN governed ⟨interp⟩ | 894.6ms | 969.0ms | 108% | 43.41 ops/CPU-ms |
...
| record-allocation | LogicN passive ⟨interp⟩ | 0.2ms | 0.0ms | 0% | — |
| record-allocation | LogicN manifest ⟨interp⟩ | 18.1ms | 16.0ms | 88% | 625.00 ops/CPU-ms |
| record-allocation | LogicN governed ⟨interp⟩ | 20.4ms | 47.0ms | 230% | 212.77 ops/CPU-ms |
...
| fibonacci-recursive | LogicN manifest ⟨interp⟩ | 62.7ms | 109.0ms | 174% | 0.01 ops/CPU-ms |
| fibonacci-recursive | LogicN governed ⟨interp⟩ | 90.8ms | 140.0ms | 154% | 0.01 ops/CPU-ms |
...
| collection-pipeline | LogicN manifest ⟨interp⟩ | 14.9ms | 16.0ms | 107% | 625.00 ops/CPU-ms |
| collection-pipeline | LogicN governed ⟨interp⟩ | 16.0ms | 15.0ms | 94% | 666.67 ops/CPU-ms |
...
| governance-cost | LogicN passive ⟨interp⟩ | 1.2ms | 0.0ms | 0% | — |
| governance-cost | LogicN manifest ⟨interp⟩ | 1.4ms | 0.0ms | 0% | — |
| governance-cost | LogicN governed ⟨interp⟩ | 1.2ms | 0.0ms | 0% | — |
...
| hardware-targets | LogicN passive ⟨interp⟩ | 9.2ms | 0.0ms | 0% | — |
| hardware-targets | LogicN manifest ⟨interp⟩ | 0.2ms | 0.0ms | 0% | — |
...
| low-memory | LogicN passive ⟨interp⟩ | 0.4ms | 0.0ms | 0% | — |
| low-memory | LogicN manifest ⟨interp⟩ | 86.4ms | 93.0ms | 108% | 107.53 ops/CPU-ms |
| low-memory | LogicN governed ⟨interp⟩ | 81.9ms | 157.0ms | 192% | 63.69 ops/CPU-ms |
...
| gpu-compute | LogicN manifest ⟨interp⟩ | 263.1ms | 250.0ms | 95% | 400.00 ops/CPU-ms |
| gpu-compute | LogicN governed ⟨interp⟩ | 268.9ms | 311.0ms | 116% | 321.54 ops/CPU-ms |
...
| matrix-multiply | LogicN passive ⟨interp⟩ | 0.1ms | 0.0ms | 0% | — |
| matrix-multiply | LogicN manifest ⟨interp⟩ | 57.6ms | 63.0ms | 109% | — |
| matrix-multiply | LogicN governed ⟨interp⟩ | 55.0ms | 78.0ms | 142% | — |
| matrix-multiply | WASM ▶ production | 1.01s | 1.01s | 101% | — |
| matrix-multiply | Deno WebGPU (NVIDIA GeForce RTX 2060) | 12.5ms | — | — | — |
| crypto-ops | LogicN passive ⟨interp⟩ | 5.2ms | 0.0ms | 0% | — |
| crypto-ops | LogicN manifest ⟨interp⟩ | 0.5ms | 0.0ms | 0% | — |
| crypto-ops | LogicN governed ⟨interp⟩ | 3.6ms | 0.0ms | 0% | — |
| text-html | LogicN passive ⟨interp⟩ | 1.3ms | 0.0ms | 0% | — |
| text-html | LogicN manifest ⟨interp⟩ | 0.3ms | 0.0ms | 0% | — |
| text-html | LogicN governed ⟨interp⟩ | 1.0ms | 0.0ms | 0% | — |
| tri-logic | LogicN passive ⟨interp⟩ | 0.9ms | 0.0ms | 0% | — |
| tri-logic | LogicN manifest ⟨interp⟩ | 817.7ms | 844.0ms | 103% | — |
| tri-logic | LogicN governed ⟨interp⟩ | 865.4ms | 891.0ms | 103% | — |
| tri-logic | WASM ▶ production | 1.16s | 1.16s | 100% | — |
| data-query | LogicN passive ⟨interp⟩ | 0.4ms | 0.0ms | 0% | — |
| data-query | LogicN manifest ⟨interp⟩ | 7.6ms | 0.0ms | 0% | — |
| data-query | LogicN governed ⟨interp⟩ | 7.9ms | 0.0ms | 0% | — |
...
| call-chain | LogicN manifest ⟨interp⟩ | 907.7ms | 938.0ms | 103% | 53.30 ops/CPU-ms |
| call-chain | LogicN governed ⟨interp⟩ | 822.3ms | 890.0ms | 108% | 56.18 ops/CPU-ms |
| nbody | Node.js | 53.2ms | 63.0ms | 118% | 104.0K ops/CPU-ms |
| nbody | Python | 1.10s | — | — | — |
| nbody | LogicN passive ⟨interp⟩ | 0.1ms | 0.0ms | 0% | — |
| nbody | LogicN manifest ⟨interp⟩ | 563.3ms | 594.0ms | 105% | 55.17 ops/CPU-ms |
| nbody | LogicN governed ⟨interp⟩ | 559.9ms | 562.0ms | 100% | 58.31 ops/CPU-ms |
| json-parse | LogicN passive ⟨interp⟩ | 0.2ms | 0.0ms | 0% | — |
| json-parse | LogicN manifest ⟨interp⟩ | 53.5ms | 63.0ms | 118% | 7.94 ops/CPU-ms |
| json-parse | LogicN governed ⟨interp⟩ | 68.0ms | 109.0ms | 160% | 4.59 ops/CPU-ms |
| mandelbrot | Rust (generic) | 140.1ms | — | — | — |
| mandelbrot | Node.js | 526.2ms | 516.0ms | 98% | 6.4K ops/CPU-ms |
| mandelbrot | Python | 12.97s | — | — | — |
| mandelbrot | LogicN passive ⟨interp⟩ | 0.1ms | 0.0ms | 0% | — |
| mandelbrot | LogicN manifest ⟨interp⟩ | 2.26s | 2.30s | 102% | 7.13 ops/CPU-ms |
| mandelbrot | LogicN governed ⟨interp⟩ | 2.06s | 2.08s | 101% | 7.88 ops/CPU-ms |
...
| binary-trees | LogicN passive ⟨interp⟩ | 0.0ms | 0.0ms | 0% | — |
| binary-trees | LogicN manifest ⟨interp⟩ | 32.5ms | 63.0ms | 194% | 2.2K ops/CPU-ms |
| binary-trees | LogicN governed ⟨interp⟩ | 36.2ms | 47.0ms | 130% | 2.9K ops/CPU-ms |
...
> Managed runtimes (Node/Python/LogicN/WASM) report it via a GC'd before/after delta; native Rust/C++
> show **~0 (native)** — no GC-managed heap. `~0` = no measurable per-op allocation (e.g. V8 tagged ints);
> a large positive value (e.g. the LogicN tree-walker boxing a value per AST node) is allocation pressure.

### compute-mix

| # | 🚦 | Runtime | Throughput | Wall | CPU | RSS | Heap/op | vs Python | vs Node |
|---|---|---|---|---|---|---|---|---|---|
| 🥇 | 🟢 | LogicN passive ⟨interp⟩ | 850.82M/s | 0.2ms | 0.0ms | 76.8MB | ~0 | 863.3× | 6.34× |
...
| 7 | ⚫ | LogicN manifest ⟨interp⟩ | 225.5K/s | 221.7ms | 234.0ms | 76.8MB | 19 B/op | 0.23× | 0.00× |
| 8 | ⚫ | LogicN governed ⟨interp⟩ | 215.2K/s | 232.3ms | 235.0ms | 76.6MB | 33 B/op | 0.22× | 0.00× |

> 🧠 **Lowest heap/op:** WASM ▶ production (~0) · **highest:** LogicN governed ⟨interp⟩ (33 B/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.
...
| 6 | ⚫ | LogicN manifest ⟨interp⟩ | 852.6K/s | 74.2ms | 125.0ms | 77.5MB | 25 B/op | 0.18× | 0.00× |
| 7 | ⚫ | LogicN governed ⟨interp⟩ | 731.8K/s | 86.4ms | 78.0ms | 77.5MB | 3 B/op | 0.16× | 0.00× |
| 8 | ⚫ | LogicN passive ⟨interp⟩ | 45.0K/s | 0.1ms | 0.0ms | 77.5MB | 3.0 KB/op | 0.01× | 0.00× |

> 🧠 **Lowest heap/op:** WASM ▶ production (~0) · **highest:** LogicN passive ⟨interp⟩ (3.0 KB/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.
...
| 6 | 🔴 | LogicN manifest ⟨interp⟩ | 47.3K/s | 889.6ms | 953.0ms | 79.3MB | 13 B/op | 0.48× | 0.02× |
| 7 | 🔴 | LogicN governed ⟨interp⟩ | 47.0K/s | 894.6ms | 969.0ms | 79.0MB | 19 B/op | 0.47× | 0.02× |
| 8 | 🔴 | LogicN passive ⟨interp⟩ | 38.0K/s | 0.1ms | 0.0ms | 79.5MB | 11.9 KB/op | 0.38× | 0.01× |

> 🧠 **Lowest heap/op:** WASM ▶ production (~0) · **highest:** LogicN passive ⟨interp⟩ (11.9 KB/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.
...
| 4 | 🟢 | LogicN passive ⟨interp⟩ | 1.01B/s | 0.2ms | 0.0ms | 79.9MB | ~0 | 191.8× | 18.1× |
| 5 | 🟢 | Node.js | 55.85M/s | 3.6ms | 0.0ms | 48.1MB | 1 B/op | 10.6× | 1.00× |
| 6 | 🔴 | Python | 5.26M/s | 38.0ms | 31.3ms | — | ~0 | 1.00× | 0.09× |
| 7 | ⚫ | LogicN manifest ⟨interp⟩ | 551.9K/s | 18.1ms | 16.0ms | 79.2MB | 71 B/op | 0.10× | 0.01× |
| 8 | ⚫ | LogicN governed ⟨interp⟩ | 489.7K/s | 20.4ms | 47.0ms | 79.9MB | 72 B/op | 0.09× | 0.01× |

> 🧠 **Lowest heap/op:** WASM ▶ production (~0) · **highest:** LogicN governed ⟨interp⟩ (72 B/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.

### fibonacci-recursive

| # | 🚦 | Runtime | Throughput | Wall | CPU | RSS | Heap/op | vs Python | vs Node |
|---|---|---|---|---|---|---|---|---|---|
| 🥇 | 🟢 | LogicN passive ⟨interp⟩ | 83.2K/s | 0.1ms | 0.0ms | 80.0MB | 6.3 KB/op | 10.3K× | 676.0× |
...
| 6 | 🟡 | LogicN manifest ⟨interp⟩ | 16.0/s | 62.7ms | 109.0ms | 79.5MB | 1326.1 KB/op | 1.98× | 0.13× |
| 7 | 🔴 | LogicN governed ⟨interp⟩ | 11.0/s | 90.8ms | 140.0ms | 79.8MB | 1046.5 KB/op | 1.36× | 0.09× |
| 8 | 🔴 | Python | 8.1/s | 2.48s | 2.48s | — | 23 B/op | 1.00× | 0.07× |

> 🧠 **Lowest heap/op:** WASM ▶ production (~0) · **highest:** LogicN manifest ⟨interp⟩ (1326.1 KB/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.
...
| 4 | 🟢 | LogicN passive ⟨interp⟩ | 1.16B/s | 0.3ms | 0.0ms | 79.4MB | ~0 | 54.2× | 17.1× |
| 5 | 🟢 | Node.js | 68.14M/s | 733.8ms | 719.0ms | 63.1MB | ~0 | 3.17× | 1.00× |
| 6 | 🟡 | Python | 21.48M/s | 2.33s | 2.33s | — | ~0 | 1.00× | 0.32× |
| 7 | ⚫ | LogicN manifest ⟨interp⟩ | 670.7K/s | 14.9ms | 16.0ms | 79.3MB | 11 B/op | 0.03× | 0.01× |
| 8 | ⚫ | LogicN governed ⟨interp⟩ | 625.0K/s | 16.0ms | 15.0ms | 80.3MB | 9 B/op | 0.03× | 0.01× |

> 🧠 **Lowest heap/op:** WASM ▶ production (~0) · **highest:** LogicN manifest ⟨interp⟩ (11 B/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.
...
| 5 | 🔴 | LogicN passive ⟨interp⟩ | 80.3K/s | 1.2ms | 0.0ms | 81.3MB | 8.1 KB/op | 2.20× | 0.04× |
| 6 | 🔴 | Python | 36.5K/s | 2.74s | 2.75s | — | ~0 | 1.00× | 0.02× |
| 7 | ⚫ | LogicN governed ⟨interp⟩ | 820.0/s | 1.2ms | 0.0ms | 81.0MB | 451.7 KB/op | 0.02× | 0.00× |
| 8 | ⚫ | LogicN manifest ⟨interp⟩ | 690.0/s | 1.4ms | 0.0ms | 82.8MB | 434.2 KB/op | 0.02× | 0.00× |

> 🧠 **Lowest heap/op:** WASM ▶ production (~0) · **highest:** LogicN governed ⟨interp⟩ (451.7 KB/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.
...
| 5 | 🟡 | LogicN passive ⟨interp⟩ | 108.8K/s | 9.2ms | 0.0ms | 80.5MB | 50 B/op | — | 0.12× |
| 6 | ⚫ | LogicN governed ⟨interp⟩ | 6.7K/s | 0.1ms | 0.0ms | 80.0MB | 35.6 KB/op | — | 0.01× |
| 7 | ⚫ | LogicN manifest ⟨interp⟩ | 4.5K/s | 0.2ms | 0.0ms | 80.3MB | 35.6 KB/op | — | 0.01× |

> 🧠 **Lowest heap/op:** WASM ▶ production (~0) · **highest:** LogicN manifest ⟨interp⟩ (35.6 KB/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.
...
| 5 | ⚪ | LogicN passive ⟨interp⟩ | 508.00M/s | 0.4ms | 0.0ms | 80.7MB | -4 B/op | 91.2× | 0.72× |
| 6 | ⚫ | Python | 5.57M/s | 1.80s | 1.80s | — | ~0 | 1.00× | 0.01× |
| 7 | ⚫ | LogicN governed ⟨interp⟩ | 122.1K/s | 81.9ms | 157.0ms | 80.5MB | 66 B/op | 0.02× | 0.00× |
| 8 | ⚫ | LogicN manifest ⟨interp⟩ | 115.7K/s | 86.4ms | 93.0ms | 80.4MB | 87 B/op | 0.02× | 0.00× |

> 🧠 **Lowest heap/op:** LogicN passive ⟨interp⟩ (-4 B/op) · **highest:** LogicN manifest ⟨interp⟩ (87 B/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.

### gpu-compute

| # | 🚦 | Runtime | Throughput | Wall | CPU | RSS | Heap/op | vs Python | vs Node |
|---|---|---|---|---|---|---|---|---|---|
| 🥇 | 🟢 | LogicN passive ⟨interp⟩ | 8.35B/s | 0.1ms | 0.0ms | 80.6MB | ~0 | 666.4× | 8.48× |
...
| 8 | ⚫ | LogicN manifest ⟨interp⟩ | 380.0K/s | 263.1ms | 250.0ms | 80.6MB | 4 B/op | 0.03× | 0.00× |
| 9 | ⚫ | LogicN governed ⟨interp⟩ | 371.8K/s | 268.9ms | 311.0ms | 80.3MB | 2 B/op | 0.03× | 0.00× |

> 🧠 **Lowest heap/op:** WASM ▶ production (~0) · **highest:** LogicN manifest ⟨interp⟩ (4 B/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.

### matrix-multiply ⚠️ (excluded — not unit-aligned)

> workload SIZE differs by runtime — LogicN/WASM n=32 (1024 cells, 32³ mul-adds), node/python/rust n=64 (4096 cells, 64³), Deno n=128 (16384 cells). Per-cell work (dot-product length n) also differs, so no shared unit is apples-to-apples until n is unified.
...
| Python | 37.1/s | 1.35s |
| LogicN passive ⟨interp⟩ | 58.7K/s | 0.1ms |
| LogicN manifest ⟨interp⟩ | 17.0/s | 57.6ms |
| LogicN governed ⟨interp⟩ | 18.0/s | 55.0ms |
...
| # | 🚦 | Runtime | Throughput | Wall | CPU | RSS | Heap/op | vs Python | vs Node |
|---|---|---|---|---|---|---|---|---|---|
| 🥇 | 🟢 | LogicN passive ⟨interp⟩ | 19.1K/s | 5.2ms | 0.0ms | 81.8MB | 5.4 KB/op | — | — |
| 🥈 | 🟡 | LogicN manifest ⟨interp⟩ | 2.0K/s | 0.5ms | 0.0ms | 81.4MB | 82.8 KB/op | — | — |
| 🥉 | 🔴 | LogicN governed ⟨interp⟩ | 274.0/s | 3.6ms | 0.0ms | 81.4MB | 204.0 KB/op | — | — |

> 🧠 **Lowest heap/op:** LogicN passive ⟨interp⟩ (5.4 KB/op) · **highest:** LogicN governed ⟨interp⟩ (204.0 KB/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.

### text-html

| # | 🚦 | Runtime | Throughput | Wall | CPU | RSS | Heap/op | vs Python | vs Node |
|---|---|---|---|---|---|---|---|---|---|
| 🥇 | 🟢 | LogicN passive ⟨interp⟩ | 75.7K/s | 1.3ms | 0.0ms | 80.8MB | -3.6 KB/op | — | — |
| 🥈 | 🔴 | LogicN manifest ⟨interp⟩ | 3.2K/s | 0.3ms | 0.0ms | 82.0MB | 113.5 KB/op | — | — |
| 🥉 | 🔴 | LogicN governed ⟨interp⟩ | 1.0K/s | 1.0ms | 0.0ms | 82.0MB | 129.7 KB/op | — | — |

> 🧠 **Lowest heap/op:** LogicN passive ⟨interp⟩ (-3.6 KB/op) · **highest:** LogicN governed ⟨interp⟩ (129.7 KB/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.

### tri-logic ⚠️ (excluded — not unit-aligned)

> incomparable workloads — LogicN main()=runBulkTri(100000) (100k triples / 300k trit-ops); node/python/rust run nested 9-element truth-table micro-benches plus a separate 10M bulk; logicnOpsPerRun=27000 (≈27 truth-table combos ×1000) corresponds to none of these. Needs a common bulk-N trit-op path on every runtime.
...
| Node.js | — | — |
| Python | — | — |
| LogicN passive ⟨interp⟩ | 113.7K/s | 0.9ms |
| LogicN manifest ⟨interp⟩ | 1.0/s | 817.7ms |
| LogicN governed ⟨interp⟩ | 1.0/s | 865.4ms |
| WASM ▶ production | 518.52M/s | 1.16s |

### data-query ⚠️ (excluded — not unit-aligned)

> incomparable — LogicN main()=filterAndCount(1000)+groupByCategory(1000)=2000 record-scans (but logicnOpsPerRun=1000 undercounts); node/python run 7 separate query micro-benches nested under results.* with no single representative. Needs a main() recount + a chosen representative query before the numbers compare.

| Runtime | Raw reported throughput (native unit — **NOT comparable**) | Wall |
|---|---|---|
| Node.js | — | — |
| Python | — | — |
| LogicN passive ⟨interp⟩ | 112.3K/s | 0.4ms |
| LogicN manifest ⟨interp⟩ | 132.0/s | 7.6ms |
| LogicN governed ⟨interp⟩ | 126.0/s | 7.9ms |

### call-chain

| # | 🚦 | Runtime | Throughput | Wall | CPU | RSS | Heap/op | vs Python | vs Node |
|---|---|---|---|---|---|---|---|---|---|
| 🥇 | 🟢 | LogicN passive ⟨interp⟩ | 4.99B/s | 0.1ms | 0.0ms | 92.8MB | ~0 | 2.4K× | 16.2× |
| 🥈 | 🟢 | Node.js | 307.73M/s | 6.5ms | 0.0ms | 46.9MB | ~0 | 147.0× | 1.00× |
| 🥉 | ⚫ | Python | 2.09M/s | 477.8ms | 468.8ms | — | ~0 | 1.00× | 0.01× |
| 4 | ⚫ | LogicN governed ⟨interp⟩ | 60.8K/s | 822.3ms | 890.0ms | 83.2MB | 24 B/op | 0.03× | 0.00× |
| 5 | ⚫ | LogicN manifest ⟨interp⟩ | 55.1K/s | 907.7ms | 938.0ms | 92.8MB | 20 B/op | 0.03× | 0.00× |

> 🧠 **Lowest heap/op:** Node.js (~0) · **highest:** LogicN governed ⟨interp⟩ (24 B/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.

### nbody

| # | 🚦 | Runtime | Throughput | Wall | CPU | RSS | Heap/op | vs Python | vs Node |
|---|---|---|---|---|---|---|---|---|---|
| 🥇 | 🟢 | LogicN passive ⟨interp⟩ | 2.24B/s | 0.1ms | 0.0ms | 93.2MB | ~0 | 1.5K× | 18.2× |
| 🥈 | 🟢 | Node.js | 123.09M/s | 53.2ms | 63.0ms | 48.3MB | ~0 | 82.9× | 1.00× |
| 🥉 | 🔴 | Python | 1.48M/s | 1.10s | — | — | 12 B/op | 1.00× | 0.01× |
| 4 | ⚫ | LogicN governed ⟨interp⟩ | 58.5K/s | 559.9ms | 562.0ms | 91.1MB | 8 B/op | 0.04× | 0.00× |
| 5 | ⚫ | LogicN manifest ⟨interp⟩ | 58.2K/s | 563.3ms | 594.0ms | 93.2MB | 36 B/op | 0.04× | 0.00× |

> 🧠 **Lowest heap/op:** Node.js (~0) · **highest:** LogicN manifest ⟨interp⟩ (36 B/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.

### json-parse

| # | 🚦 | Runtime | Throughput | Wall | CPU | RSS | Heap/op | vs Python | vs Node |
|---|---|---|---|---|---|---|---|---|---|
| 🥇 | 🟢 | LogicN passive ⟨interp⟩ | 47.30M/s | 0.2ms | 0.0ms | 92.9MB | 8 B/op | 74.5× | 14.3× |
| 🥈 | 🟢 | Node.js | 3.32M/s | — | — | — | — | 5.23× | 1.00× |
| 🥉 | 🟡 | Python | 635.0K/s | — | — | — | 1 B/op | 1.00× | 0.19× |
| 4 | ⚫ | LogicN manifest ⟨interp⟩ | 9.3K/s | 53.5ms | 63.0ms | 92.7MB | 3.7 KB/op | 0.01× | 0.00× |
| 5 | ⚫ | LogicN governed ⟨interp⟩ | 7.4K/s | 68.0ms | 109.0ms | 99.0MB | 6.4 KB/op | 0.01× | 0.00× |

> 🧠 **Lowest heap/op:** Python (1 B/op) · **highest:** LogicN governed ⟨interp⟩ (6.4 KB/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.

### mandelbrot

| # | 🚦 | Runtime | Throughput | Wall | CPU | RSS | Heap/op | vs Python | vs Node |
|---|---|---|---|---|---|---|---|---|---|
| 🥇 | 🟢 | LogicN passive ⟨interp⟩ | 1.20B/s | 0.1ms | 0.0ms | 94.4MB | ~0 | 4.7K× | 192.0× |
...
| 5 | ⚫ | LogicN governed ⟨interp⟩ | 7.9K/s | 2.06s | 2.08s | 93.9MB | 83 B/op | 0.03× | 0.00× |
| 6 | ⚫ | LogicN manifest ⟨interp⟩ | 7.2K/s | 2.26s | 2.30s | 94.4MB | 209 B/op | 0.03× | 0.00× |

> 🧠 **Lowest heap/op:** Python (~0) · **highest:** LogicN manifest ⟨interp⟩ (209 B/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.
...
| # | 🚦 | Runtime | Throughput | Wall | CPU | RSS | Heap/op | vs Python | vs Node |
|---|---|---|---|---|---|---|---|---|---|
| 🥇 | 🟢 | LogicN passive ⟨interp⟩ | 10.90B/s | 0.0ms | 0.0ms | 93.7MB | ~0 | 2.0K× | 143.5× |
...
| 5 | 🔴 | LogicN manifest ⟨interp⟩ | 4.18M/s | 32.5ms | 63.0ms | 93.7MB | 8 B/op | 0.78× | 0.06× |
| 6 | 🔴 | LogicN governed ⟨interp⟩ | 3.75M/s | 36.2ms | 47.0ms | 93.7MB | 15 B/op | 0.70× | 0.05× |

> 🧠 **Lowest heap/op:** Python (~0) · **highest:** LogicN governed ⟨interp⟩ (15 B/op). Native Rust/C++ allocate ~0 (no GC heap); a positive figure is GC-managed allocation pressure.
...
|---|---|---|---|---|---|---|
| 🥇 | 🟢 | LogicN passive ⟨interp⟩ | 🖥️ CPU (cpu) | 8.35B/s | 0.1ms | 8.48× |
...
| 8 | ⚫ | LogicN manifest ⟨interp⟩ | 🖥️ CPU (cpu) | 380.0K/s | 263.1ms | 0.00× |
| 9 | ⚫ | LogicN governed ⟨interp⟩ | 🖥️ CPU (cpu) | 371.8K/s | 268.9ms | 0.00× |
...
**collection-pipeline: the old "LogicN wins 43×" was a UNIT bug, now fixed:**
- That claim compared LogicN's *elements/sec* against the other languages' *whole-pipeline-passes/sec* —
...
|---|---|---|---|---|---|---|---|---|---|---|
| **compute-mix** | LogicN passive ⟨interp⟩ | 7× slower | 6× slower | 6× slower | **863× slower** | **🏆 winner** | **3.8K× slower** | **4.0K× slower** | 7× slower | — |
...
| **fibonacci-recursive** | LogicN passive ⟨interp⟩ | **168× slower** | **168× slower** | **676× slower** | **10.3K× slower** | **🏆 winner** | **5.2K× slower** | **7.6K× slower** | 2× slower | — |
...
| **gpu-compute** | LogicN passive ⟨interp⟩ | 7× slower | 7× slower | 8× slower | **666× slower** | **🏆 winner** | **22.0K× slower** | **22.4K× slower** | 5× slower | **2.1K× slower** |
| **crypto-ops** | LogicN passive ⟨interp⟩ | — | — | — | — | **🏆 winner** | 10× slower | **70× slower** | — | — |
| **text-html** | LogicN passive ⟨interp⟩ | — | — | — | — | **🏆 winner** | **23× slower** | **76× slower** | — | — |
| **call-chain** | LogicN passive ⟨interp⟩ | — | — | **16× slower** | **2.4K× slower** | **🏆 winner** | **90.6K× slower** | **82.1K× slower** | — | — |
| **nbody** | LogicN passive ⟨interp⟩ | — | — | **18× slower** | **1.5K× slower** | **🏆 winner** | **38.6K× slower** | **38.3K× slower** | — | — |
| **json-parse** | LogicN passive ⟨interp⟩ | — | — | **14× slower** | **74× slower** | **🏆 winner** | **5.1K× slower** | **6.4K× slower** | — | — |
| **mandelbrot** | LogicN passive ⟨interp⟩ | — | **51× slower** | **192× slower** | **4.7K× slower** | **🏆 winner** | **165.0K× slower** | **150.6K× slower** | — | — |
| **spectral-norm** | Rust (generic) | — | **🏆 winner** | 2× slower | **139× slower** | — | — | — | — | — |
| **binary-trees** | LogicN passive ⟨interp⟩ | — | **535× slower** | **143× slower** | **2.0K× slower** | **🏆 winner** | **2.6K× slower** | **2.9K× slower** | — | — |
