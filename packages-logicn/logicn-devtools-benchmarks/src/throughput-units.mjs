 ...
 *
 * ── The bug this fixes ──────────────────────────────────────────────────────
 * compare.mjs historically read LogicN's `logicnOpsPerSecond` (INNER-ops/sec —
 ...
 * compared inner-ops/sec against whole-calls/sec, producing false "LogicN wins":
 *   nbody showed LogicN 62.7K/s "beating" Node 3.7K/s — but Node does
...
// Native (non-LogicN, non-WASM) runtime keys handled by each spec's `native()`.
const LOGICN_INTERP = new Set(["logicnGoverned", "logicnManifest"]);
...
function anyReportedRate(r) {
  ...
  if (num(r.warmCallsPerSecond) != null) return true;   // LogicN passive
  return false;
}

// ── per-benchmark unit specs ─────────────────────────────────────────────────
// N           = inner ops one whole call / one LogicN main() performs (canonical).
...
// native(rt,r)= inner-ops/sec for a non-LogicN, non-WASM runtime (node/python/
//               rust/cpp/deno), or null if absent. LogicN tiers + WASM are handled
...
const SPECS = {
  ...
  // .tmf creation — Node column = the @logicn/ext-tmf engine; python/rust = byte-exact
  ...
  // native framework vs middleware — Node column = LogicN App Kernel; python = sync
  ...
  "tri-logic": {
    ...
    reason: "incomparable workloads — LogicN main()=runBulkTri(100000) (100k triples / 300k trit-ops); " +
  ...
  "data-query": {
    ...
    reason: "incomparable — LogicN main()=filterAndCount(1000)+groupByCategory(1000)=2000 record-scans " +
