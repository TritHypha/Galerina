...
// Mirrors benchmark.lln, python.py and bench.rs EXACTLY. The only negative division
// numerator (2*zr*zi) is split into sign+magnitude so EVERY integer division has a
// non-negative numerator → Math.trunc == Python // == Rust / == LogicN /, making the
...
function run(iterations) {
  ...
  return {
    ...
    notes: ["Scaled-integer Mandelbrot escape-time — checksum matches Python, Rust and LogicN bit-for-bit"],
  };
}
