...
// Mirrors benchmark.lln, node.mjs and python.py EXACTLY. The only negative division
// numerator (2*zr*zi) is split into sign+magnitude so every numerator is non-negative,
// making Rust `/` (trunc toward zero) agree with Python //, JS Math.trunc and LogicN /.
