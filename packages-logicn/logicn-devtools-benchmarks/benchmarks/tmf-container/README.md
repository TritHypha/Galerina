...
|---|---|
| **Node.js** | **LogicN's shipped `@logicn/ext-tmf` engine** (`dist/index.js`, `writeTmf`). The engine is pure TypeScript-on-Node — there is **no `.lln` execution path** for `.tmf` creation — so the engine's throughput *is* the Node.js row. There is deliberately no LogicN-interpreter column here. |
...
- **This is a SHAKE256 + serialization race across language runtimes**, not "LogicN vs
  the world" — the LogicN engine *is* the Node implementation. Label it as such.
