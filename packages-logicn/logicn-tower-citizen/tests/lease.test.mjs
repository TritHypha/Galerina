// packages-logicn/logicn-tower-citizen/tests/lease.test.mjs — FAIL-CLOSED TTL capability lease (G6).
//
...
// carrying + sinking LLN-GOV-3VL-001; purity/determinism (same (lease, now) → same decision, no
...
test("AT notAfter → DENY (half-open window, hard expiry)", () => {
  ...
  assert.equal(d.diagnostic, null, "expiry is an ordinary DENY, not LLN-GOV-3VL-001");
...
test("malformed / absent lease → INDETERMINATE collapse → deny-by-default, audited", () => {
  ...
  assert.ok(d.diagnostic && d.diagnostic.code === GOV_3VL_DIAGNOSTIC, "carries LLN-GOV-3VL-001");
