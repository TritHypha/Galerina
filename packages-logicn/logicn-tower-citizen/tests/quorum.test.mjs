// packages-logicn/logicn-tower-citizen/tests/quorum.test.mjs — distinct-signer M-of-N quorum (G2 core).
//
...
// equivocation and any malformed input collapse to INDETERMINATE carrying + sinking LLN-GOV-3VL-001;
...
test("clean shortfall (well-formed, < M approvals) → DENY, NOT a diagnostic", () => {
  ...
  assert.equal(d.diagnostic, null, "a clean shortfall is an ordinary DENY, not LLN-GOV-3VL-001");
...
test("equivocation (one signer, conflicting verdicts) → INDETERMINATE + LLN-GOV-3VL-001", () => {
  let sunk = null;
