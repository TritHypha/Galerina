 ...
 * [[logicn-key-custody-rotation-decision]]). Each `SignerVote.verdict` is expected to be
 * the result of an upstream signature/attestation verification done on binary silicon
 * (LLN-SUBSTRATE-001); this primitive treats it as an already-decided trit.
 *
 ...
 *    INDETERMINATE → denied, audited LLN-GOV-3VL-001. There is no path by which bad
 *    input authorizes.
 ...
 *    malformed/equivocating input raises LLN-GOV-3VL-001.
 ...
 * carrying LLN-GOV-3VL-001 (also forwarded to `onDiagnostic` if provided). Pure.
 */
