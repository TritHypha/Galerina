/**
 * verdict.ts — the K3 verdict algebra, re-exported as logicn-auth's vocabulary.
 *
 * Authentication/authorization in LogicN is three-valued, not boolean. Every auth
 * FACTOR this package computes is a Kleene-K3 trit (the shipped @logicn/tower-citizen
 * `Verdict`):
 *
 ...
 * boundary collapse VERBATIM from tower-citizen so a consumer of logicn-auth can
 * type and interpret a factor without reaching across into another package — the
 * same courtesy @logicn/core-network's cert-gate extends. We never re-implement the
