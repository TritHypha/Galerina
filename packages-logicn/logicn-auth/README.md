...
> `logicn-auth` lifts the authentication/authorization FACTORS out of the App
...
> core. It sits **above** the LogicN language + core runtime, beside
> `logicn-framework-app-kernel` and `logicn-core-network`.

`logicn-auth` computes the **auth/identity FACTORS** that the App Kernel folds at
...
This is the load-bearing design rule. **logicn-auth provides the factors; the App
...
```text
   ...
   └───────────────────────────┬─────────────────────────────────┘
                               ...
                               ▼
   ┌──────────────── logicn-framework-app-kernel (DECISION) ──────┐
...
**delegated verbatim** to the shipped cert-gate in `@logicn/core-network`
...
> library's outputs — no ASN.1 parsing, path-building, or signature math. logicn-auth
...
- **Single source of truth.** The channel factor *delegates* to the shipped
  `certGate`; the verdict algebra is *re-exported* from `@logicn/tower-citizen`.
...
- **The kernel keeps the decision.** logicn-auth never collapses a verdict to an
...
`@logicn/core-network` and `@logicn/tower-citizen` first; the kernel-integration
test additionally exercises `@logicn/framework-app-kernel` when its `dist/` is
present (and skips cleanly when it is not).

## Layout

```text
package.json          @logicn/auth descriptor (file: deps on core-network + tower-citizen)
...
- `packages-logicn/logicn-framework-app-kernel` — the kernel that folds these factors
- `packages-logicn/logicn-core-network` — the shipped `certGate` (TLSTP S1)
- `packages-logicn/logicn-tower-citizen` — the K3 verdict algebra
- `docs/Knowledge-Bases/logicn-tlstp-s1-cert-gate.md` — the cert-gate spec
