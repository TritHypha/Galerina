...
| **TLSTP — zero-middleware** | Routes *around* the OS kernel: the host writes raw encrypted packets as **unparsed byte-arrays** straight into WASM linear memory; **decryption happens strictly inside the WASM sandbox** — the kernel never sees plaintext. | ◑ 5 border decision surfaces twinned (`.fungi`, fail-closed): cert/channel gate · CORS admission · inbound admission + rate-limit · defensive controls (trusted-proxy · uniform responses · opaque ids · bounded pagination) · degrade-only telemetry self-throttle · in-sandbox decryption = design intent (DSS.wasm TCB #102-106) |
