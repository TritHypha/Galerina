...
| **TLSTP — zero-middleware** | Routes *around* the OS kernel: the host writes raw encrypted packets as **unparsed byte-arrays** straight into WASM linear memory; **decryption happens strictly inside the WASM sandbox** — the kernel never sees plaintext. | ◑ S1 cert/channel gate twinned (`.fungi`) · in-sandbox decryption = design intent (DSS.wasm TCB #102-106) |
