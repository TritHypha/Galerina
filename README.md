...
| **Tests — full suite** | 100% | **95/95 packages · 7,526 tests · 0 failures** |
...
| **Type checker / Effect checker** | 100% | *derived* from twin diagnostic-code parity — the full TYPE/EFFECT diagnostic charter mirrored with emit+name+severity parity (FUNGI-TYPE-032 twinned; last open rung closed) |
...
| **Stage-B self-hosting — WASM execution (P9)** | in progress | **3 of 7 stage twins now RUN as real signed WASM (R2)** — lexer, parser, and **gir-emitter** (gir-emitter unblocked 2026-07-19 by the P9 brick-2 concretization: its `Array<Auto>` AST params typed to the parser's `FlowDecl`/`Expr`/`Stmt`, clearing the #100 erasure trap; `audit-stage-execution.mjs` baseline 4→3). **R3 byte-parity proven (Stage-A ≡ Stage-B real WASM behind #105):** lexer `tokenize` · the full parser ladder (`parseParams → parseExpr → parseStmt → parseBlock → parseFlows`, 53 differential tests) · gir-emitter `emitGIRModule`. Remaining: type-checker · effect-checker · governance-verifier trap at compile, runtime at execution — the same #100 erasure, cleared the same way |
...
```bash
# Tests — core suite (4 packages) / full suite (95 packages, 7,526 tests)
