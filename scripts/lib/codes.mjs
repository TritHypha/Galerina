...
// (SPORE-PROFILE-005B → 005, so a shipped code VANISHED and merged into another), multi-segment codes
// were dropped (SPORE-GOV-3VL-001, SPORE-CRYPTO-PQ-001, SPORE-SYNTAX-LEGACY-003), and doc ranges
// (SPORE-ACCESS-001-002) were captured as phantom single codes. Import from here; never hand-roll again.
//
//   SPORE: family (one or more alpha/num segments) then a final -<digits> with an OPTIONAL trailing letter.
//   ERR: ERR_ then segments; must END on a non-underscore char (rejects ERR_AI_ wildcard prefixes).
export const CODE_SRC = "SPORE-[A-Z0-9]+(?:-[A-Z0-9]+)*-[0-9]+[A-Z]?|ERR_[A-Z0-9_]*[A-Z0-9]";

/** Anchored, non-global — test whether a whole token IS a code. */
export const CODE_TEST = new RegExp(`^(?:${CODE_SRC})$`);

// A doc RANGE expression like `SPORE-ACCESS-001-002` (two trailing numeric segments) is not a single code.
const RANGE = /-[0-9]+-[0-9]+$/;
...
export const familyOf = (c) => (c.startsWith("ERR_") ? "ERR_*" : (c.match(/^SPORE-([A-Z0-9]+)-/)?.[1] ?? "?"));
export const nsOf = (c) => (c.startsWith("ERR_") ? "ERR" : "SPORE");
