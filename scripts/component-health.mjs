...
const ZERO_TRUST = [
  ...
  { boundary: "TLSTP — zero-middleware", pct: 38, status: "◑ 4 core-network border DECISION surfaces twinned (.fungi, all fail-closed): cert-gate (S1 K3 pin·chain·expiry·revocation vAnd, unknown→DENY) · cors-policy (deny-by-default read admission) · inbound-guard (deny-by-default port admission + fail-closed rate-limit verdict) · defensive-controls (RD-0325/0326: verified-trusted-proxy · uniform not-found≡forbidden · opaque-id · bounded pagination) · in-sandbox decryption = target (DSS.wasm TCB, #143 switch)" },
];
