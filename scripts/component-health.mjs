...
const BUILD_PROGRESS = [
  ...
  { layer: "Stage-B self-hosting — WASM execution (P9)", status: "in progress — 2 of 7 stages RUN · ★ 3 of 7 TRAP. PROVEN at R3 (byte-parity ≡ Stage-A through real WASM behind #105): lexer `tokenize` · the whole PARSER ladder to its entry point — parseParams (flat) → parseExpr (recursive) → parseStmt (self+mutual) → parseBlock → parseFlows (2026-07-17, 53 differential tests, all first-run green); the recursive AST readback needed NO new ABI (that was an assertion; it is now executed). ★ TRAP AT R2 — type-checker · effect-checker · governance-verifier: each is R0-clean, R0-assembles, R1-#105-admits AND EXPORTS, then throws `unreachable` on one trivial flow. ROOT CAUSE (isolated one-variable-at-a-time, 20-line repro with R&D): reading a RECORD FIELD off an Option payload obtained from `.get()` on an `Array<Auto>` — Array<Auto> alone is fine, get+field on a CONCRETE array is fine, the two TOGETHER erase the element type so the field offset is unknown at lowering and the emitter falls through to `unreachable`. That is TASK #100 (Option<Record> payload), now P9's blocker for 3 stages; the checkers take Array<Auto> per their own headers 'to avoid generic syntax constraints in Stage B param declarations' — the workaround IS the bug. ALSO: the stages cannot be concatenated — `appendAll` is defined in both type-checker and governance-verifier (`paramNames` in type-checker and gir-emitter), the compiler accepts duplicate FLOW names (FUNGI-NAME-002 covers bindings only), and it surfaces only as a WASM `Duplicate export name` at instantiate — so the whole-compiler bundle P9 ends at cannot be built today. ⚠ THE ROW WAS THE LESSON: 'all 7 twins build (R0) + admit (R1), flows callable' stood from 2026-07-12 and read as 'the stages work'. They ARE callable. Three do not run. R1 asks 'does it export?', R3 asks 'does it agree?' — nothing asked 'does it RUN?'. UNSWEPT (declared): gir-emitter (takes Array<Stmt>, off the flows chain) · runtime" },
