...
class Parser {
  ...
  private parseParam(): AstNode | undefined {
    ...
    if (nameTok.kind !== "identifier") {
      if (nameTok.kind === "keyword") {
        // DX: a reserved Galerina keyword (e.g. `governance`, `policy`, `authority`) in parameter
        // position would otherwise raise the generic "Expected parameter name" without saying WHY.
        this.emitUnexpected(
          `Expected a parameter name, but "${nameTok.value}" is a reserved Galerina keyword and ` +
  ...
  private parseIfStmt(): AstNode {
    ...
    if (this.currentIs("keyword", "else")) {
      this.advance();
      this.skipNewlines();
      // `else if` is NOT allowed in Galerina. Use `match` for multi-branch logic.
      // Rationale: `else if` is TypeScript/JavaScript baggage. Galerina uses `match`
  ...
  private parseMatchArm(): AstNode | undefined {
    ...
    // Used for ordered threshold matching — Galerina alternative to else-if chains.
    //   match score {
  ...
  private parsePostfix(): AstNode {
    ...
    while (true) {
      ...
      // `::` path separator: `module::submodule::symbol` — canonical Galerina module path syntax.
  ...
  private parseContractDecl(): AstNode {
    ...
    while (!this.currentIs("symbol", "}") && !this.isEof()) {
      ...
      // live in a non-core `galerina-ext-*` driver. See galerina-design-secrets-epilogue-blocks.md.
   ...
   * the non-core ext driver (galerina-ext-secrets-vault); core only retains the declared policy so
