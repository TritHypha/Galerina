// =============================================================================
// Galerina — .lmanifest Generator (DRCM Phase 1 task #33 + Phase 3 task #67)
//
...
//
// SERIALISATION — Two formats (dual output per galerina-cbor-manifest-spec.md):
//
...
//                      Ed25519 keys are configured (`galerina keygen`); ML-DSA-65 is gated on key
...
//   - No floating-point (Galerina uses integers for limits/economics — no IEEE drift)
//
// Security hardening (galerina-cbor-manifest-spec.md):
...
//   - galerina-engineering-goals.md (Pattern 9)
//   - galerina-deterministic-runtime-containment.md (Phase 3)
 ...
 *   - Nesting depth limit: 8 (Billion Laughs mitigation per galerina-cbor-manifest-spec.md)
 *
 * Security hardening:
...
export function encodeCBOR(value: unknown, depth = 0): Uint8Array {
  ...
  if (typeof value === "number") {
    if (!Number.isInteger(value)) throw new Error("CBOR: non-integer numbers not allowed in Galerina manifests");
...
export function generateManifest(
  ...
  // never carries an unenforced capability claim. See galerina-cbor-manifest-spec.md §415.
  ...
  // stays in galerina-ext-secrets-vault (govern-don't-absorb); core only RETAINS the declared policy
...
/**
 * CBOR Tag Registry (tags 400-499 are Galerina-reserved per galerina-cbor-manifest-spec.md).
 * Used as metadata reference until binary CBOR encoder ships in task #67.
 */
export const GALERINA_CBOR_TAGS = {
  Capability:          400,
  Effect:              401,
  SecretHandle:        402,
  ProofObligation:     403,
  ...
  // deferred — see galerina-cbor-manifest-spec.md). AuditEvent (410) stays RUNTIME-ONLY by
