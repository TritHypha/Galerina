// =============================================================================
// Galerina Phase 17A — Package Manifest Resolver
//
// Resolves package.galerina.yaml manifest files from package directories.
...
// that handles the subset of YAML used in package.galerina.yaml files.
// =============================================================================
 ...
 * Mirrors the admission-gate regex in galerina-framework-app-kernel/src/fuse-loader.ts.
...
export function checkPackageProvenance(
  ...
  if (!hasValidContentHash(manifest.hash)) {
    diags.push({
      ...
      suggestedFix: "Add 'hash: sha256:<64 hex>' to the package manifest. Run 'galerina package hash' to generate it.",
    });
  }
...
// Minimal YAML parser
//
// Handles the strict subset used by package.galerina.yaml:
//
//   name: "@myorg/types"
//   version: "0.1.0"
//   exports:
...
/**
 * Loads and parses a package.galerina.yaml manifest from `packagePath`.
 *
 * Returns `undefined` if the file does not exist or cannot be parsed.
 */
export function loadPackageManifest(packagePath: string): PackageManifest | undefined {
  const manifestPath = join(packagePath, "package.galerina.yaml");
