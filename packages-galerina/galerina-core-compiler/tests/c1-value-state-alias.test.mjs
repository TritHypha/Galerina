// C1 (threat-model) completion — the value-state TAINT sink (SPORE-VALUESTATE-008) must also resolve
...
test("C1 taint sink: transitive alias chain `let y = x; let x = AuditLog; y.write` flags it", () => {
  assert.ok(has(diags(wrap("  let x = AuditLog\n  let y = x\n  y.write(msg)")), "SPORE-VALUESTATE-008"));
});
