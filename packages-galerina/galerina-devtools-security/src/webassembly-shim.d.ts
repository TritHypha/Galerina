...
// security tool and must not gain ambient browser globals that could mask non-portable
...
// (`skipLibCheck`) for a security-tier package, we PROVIDE exactly the narrow
// `WebAssembly` type surface those declarations reference. This package never
// instantiates WebAssembly itself; these types exist purely so the consumed compiler
// declarations type-check. Shapes mirror TypeScript's own `lib.dom.d.ts`.
