...
export function guardInboundRequest(req: InboundRequest, policy: InboundGuardPolicy): InboundDecision {
  if (!Number.isInteger(req.port) || req.port < 0 || req.port > 65535) {
    return { allowed: false, reason: `invalid inbound port ${req.port}`, code: "Galerina_NETWORK_INBOUND_BAD_PORT" };
  }
  ...
  return { allowed: false, reason: `inbound port ${req.port} matched no allow rule (deny-by-default)`, code: "Galerina_NETWORK_INBOUND_DENY_DEFAULT" };
}
...
export class RateLimiter {
  ...
  consume(key: string, rule: RateLimitRuleLike, nowMs: number): RateLimitResult {
    ...
    if (parsed === null) {
      return { allowed: false, remaining: 0, resetMs: nowMs, code: "Galerina_NETWORK_RATELIMIT_UNPARSEABLE" };
    }
    ...
    if (w.count >= count) {
      return { allowed: false, remaining: 0, resetMs: w.start + windowMs, code: "Galerina_NETWORK_RATELIMIT_EXCEEDED" };
    }
    w.count += 1;
