...
Galerina is a governance-first programming language designed to make execution
...
diagnostics, and the prototype compiler that makes Galerina runnable today on
Node.js.

---

> **⚠️ STATUS CORRECTION (updated 2026-06-23).** Much of *this package* README describes an early
> "prototype (v0.1-beta), Phase 4 next" framing that is **out of date**. `galerina-core` is the
> **language-specification / foundation** package; the **active production compiler** is
> [`galerina-core-compiler`](../galerina-core-compiler) (full lexer → parser → type/effect/governance
...
> `galerina new app` scaffolder, the governed package resolver, and the 0091 base packages
...
Galerina's execution model follows a four-stage semantic pipeline:
...
Galerina runs on **Node.js 18+**. The prototype compiler is a plain Node.js
script. No native toolchain, no LLVM, no Rust required.

```bash
cd packages-galerina/galerina-core
...
Galerina is a **language-design and prototype project**. It is not a production
compiler.
...
The prototype compiles Galerina source through a TypeScript/Node.js layer. This
...
Each layer provides its own safety guarantees. Galerina's layer is the most
...
```
Galerina Syntax      → intended safety model (ownership, effects, intent)
...
The eventual goal (Stage B) is for Galerina to compile itself, replacing the
TypeScript layer with a Galerina-written compiler targeting CPU binary, WASM
...
Galerina flows declare their **safety level** and their **effects**. The compiler
...
Galerina has three binding forms, each with a different mutability contract that
...
bytes. Galerina marks these bindings with a **safety prefix** at the point they
enter the program.
...
Galerina has a hybrid ownership model. Every value has an explicit ownership state.
...
Stage A — TypeScript / Node.js Runtime                  ⬜ post-foundation
  ...
  Galerina source is compiled to governed TypeScript output.
  ...
  The Galerina layer provides the strongest, most explicit guarantees.
...
Stage B — Galerina Compiles Itself                        ⬜ long-term
  The Galerina compiler and runtime are rewritten in Galerina.
  The TypeScript bootstrap layer is no longer needed for production.
  Galerina becomes self-hosting: the language proves its own model.
  ...
  Self-hosting is the maturity gate: if Galerina can compile Galerina
...
galerina-core-vector / galerina-core-compute
  Vectors, matrices, tensors, compute planning, target selection.

galerina-target-cpu / galerina-target-wasm   ← v1 active targets
  CPU capability detection, fallback, WASM planning.

galerina-target-gpu / galerina-ai / galerina-*  ← post-v1
...
While the real compiler is being built, Galerina programs run via Node.js:
...
Each layer contributes its own safety guarantees. Galerina's layer is the most explicit — ownership, effects, intent. The layers below provide managed memory and runtime safety inherited from the JS ecosystem.

**Key boundaries:**

- `galerina-core` is the language. It must not become a web framework.
- `galerina-core-compiler` is the only package that owns diagnostic emission.
...
package governance           tracks authority and effects introduced by dependencies
    ↓
build-time explainability    `galerina explain <flow>` — live queryable system model
    ↓
...
Galerina is designed to be readable by AI tools without requiring large context
windows.
...
Galerina is licensed under the Apache License 2.0. See `LICENSE`, `LICENCE.md`
and `NOTICE.md`.
