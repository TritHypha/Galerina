...
function buildOperation(
  ...
  if (authRequired) {
    ...
    // surfaced via x-galerina-scopes + the description instead.
  ...
  const operation: OperationObject = {
    ...
    responses,
    security,
    // governance provenance (x-galerina-*; template-literal index signature)
    "x-galerina-handler": handler,
    "x-galerina-rate-limit": policy.limits.rate,
    "x-galerina-max-concurrent": policy.limits.maxConcurrent,
    "x-galerina-timeout-ms": policy.limits.timeoutMs,
    "x-galerina-memory-bytes": policy.limits.memoryBytes,
    ...(acceptsBody
      ? {
          "x-galerina-max-body-bytes": policy.body.maxSizeBytes,
          "x-galerina-unknown-fields": policy.body.unknownFields,
          "x-galerina-duplicate-keys": policy.body.duplicateKeys,
        }
      : {}),
    ...(authRequired && scopes.length > 0 ? { "x-galerina-scopes": [...scopes] } : {}),
    ...(policy.relaxations.length > 0 ? { "x-galerina-relaxations": [...policy.relaxations] } : {}),
    ...(policy.appliedDefaults.length > 0 ? { "x-galerina-applied-defaults": [...policy.appliedDefaults] } : {}),
  };
...
/**
 * Generate a valid OpenAPI 3.x document from a Galerina app's governed routes.
 *
