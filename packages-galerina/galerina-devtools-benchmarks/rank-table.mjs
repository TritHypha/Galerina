// One-off: rank each benchmark's runtimes by throughput → winner / runner-up / Galerina rank.
...
console.log("BENCH | WINNER | RUNNER-UP | GALERINA_RANK | all(desc)");
for (const e of arr) {
  const b = e.benchmark;
  const res = e.results || {};
  let rows = Object.entries(res).map(([rt, o]) => [rt, perSec(o)]).filter(([, v]) => v != null).sort((a, b) => b[1] - a[1]);
  if (!rows.length) continue;
  const li = rows.findIndex(([rt]) => /galerina/i.test(rt));
